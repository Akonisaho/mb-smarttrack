...
import {
  ...
  fetchUndertakings, saveUndertaking, fulfillUndertaking, deleteUndertaking,
  fetchClientCommunications, saveClientCommunication, deleteClientCommunication,
  updateMatter,
  searchAll
} from '../lib/supabase';
...
function appIcon(n){ n=(n||'').toLowerCase(); if(n.includes('phone')||n.includes('call'))return'ðŸ“ž'; if(n.includes('word'))return'ðŸ“'; if(n.includes('excel'))return'ðŸ“Š'; if(n.includes('outlook'))return'ðŸ“§'; if(n.includes('teams'))return'ðŸ’¬'; if(n.includes('chrome')||n.includes('edge'))return'ðŸŒ'; if(n.includes('acrobat'))return'ðŸ“„'; if(n.includes('powerpoint'))return'ðŸ“‘'; if(n.includes('explorer'))return'ðŸ“'; if(n.includes('code'))return'ðŸ’»'; return'ðŸ–¥ï¸'; }
...
function printTrustStatement(matter, transactions) {
  ...
  const rows=transactions.map(t=>{
    ...
    return `<tr><td>${fmtDate(t.date)}</td><td style="text-transform:capitalize">${t.type}</td><td>${t.receipt_no||t.reference||'â€”'}</td><td>${t.narration||''}</td><td align="right" style="color:#dc2626">${!isR?fmtR(t.amount):''}</td><td align="right" style="color:#16a34a">${isR?fmtR(t.amount):''}</td><td align="right" style="font-weight:700;color:${running>=0?'#16a34a':'#dc2626'}">${fmtR(running)}</td></tr>`;
  }).join('');
  ...
  <div class="top"><div><div class="logo">M<span>B</span></div><div style="font-size:11px;color:#999;margin-top:2px">Motsoeneng Bill</div></div><div style="text-align:right"><h2>TRUST ACCOUNT STATEMENT</h2><div style="font-size:11px;color:#999">Matter: ${matter?.id} Â· Generated: ${fdate(new Date().toISOString().split('T')[0])}</div></div></div>
  <div class="info"><div><div class="lbl">Client</div><div class="val">${matter?.client||'â€”'}</div></div><div><div class="lbl">Matter</div><div class="val">${matter?.name||'â€”'}</div></div><div><div class="lbl">Current trust balance</div><div class="val" style="color:#16a34a">${fmtR(running)}</div></div><div><div class="lbl">Total transactions</div><div class="val">${transactions.length}</div></div></div>
  ...
  <div class="foot">Motsoeneng Bill Â· VAT: 4100000000 Â· FNB 62000000000 Â· Branch: 250655 Â· accounts@mb.co.za</div>
...
function downloadPDF(inv, acts) {
  ...
  <div class="top"><div><div class="logo">M<span>B</span></div><div class="logo-sub">Motsoeneng Bill</div><div class="logo-sub">The Quality of Now</div></div><div class="right"><h1>TAX INVOICE</h1><p>${inv.id}</p><p>Issued: ${fdate(new Date().toISOString().split('T')[0])}</p></div></div>
  ...
  <div class="foot">Motsoeneng Bill | VAT Reg No: 4100000000<br>FNB Account: 62000000000 | Branch: 250655<br>accounts@mb.co.za | www.mb.co.za<br><em>This invoice is computer generated and valid without a signature.</em></div>
  <script>window.onload=function(){window.print();}<\/script></body></html>`;
  const w=window.open('','_blank','width=920,height=720'); w.document.write(html); w.document.close();
}
...
function DonutChart({segments,size=140}){
  ...
  const arcs=segments.map(seg=>{ const p=seg.value/total,a1=angle,a2=angle+p*360; angle=a2; return{...seg,pct:Math.round(p*100),pctExact:seg.value/total*100}; });
  return(<svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}><circle cx={cx} cy={cy} r={r} fill="none" stroke="#1A1A1A" strokeWidth={sw}/>{arcs.map((a,i)=>(<circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={a.color} strokeWidth={sw} strokeDasharray={`${a.pctExact*2.827} 282.7`} strokeDashoffset={`${282.7*(1-arcs.slice(0,i).reduce((s,x)=>s+x.pctExact,0)/100)}`} strokeLinecap="butt"/>))}<text x={cx} y={cy-5} textAnchor="middle" fill="#F0F0F0" fontSize="13" fontWeight="700">{Math.round(arcs.find(a=>a.color==='#6CC04A')?.pctExact||0)}%</text><text x={cx} y={cy+10} textAnchor="middle" fill="#555" fontSize="9">billable</text></svg>);
}
...
const rFormDirty=useRef(false);
  ...
  function handleCSVImport(e){
    ...
    reader.onload=(ev)=>{
      try{
        ...
        showTrustAlert(`âœ“ Imported ${parsed.length} bank statement lines`,'success');
  ...
  async function postReceipt(){
    ...
    showTrustAlert(`âœ“ Receipt ${receiptNo} posted â€” ${fmtR(amount)} credited to ${matters.find(m=>m.id===rForm.matterId)?.client||rForm.matterId}`,'success');
  ...
  async function postPayment(){
    ...
    if(amount>bal){ showTrustAlert(`âœ— Insufficient balance. Available: ${fmtR(bal)} â€” Requested: ${fmtR(amount)}. Payment blocked.`,'error'); return; }
    setTrustSaving(true);
    ...
    if(error){ showTrustAlert('Error: '+error.message,'error'); setTrustSaving(false); return; }
    showTrustAlert(needsApproval?`â³ Payment of ${fmtR(amount)} submitted for partner approval.`:`âœ“ Payment of ${fmtR(amount)} to ${pForm.payee} posted.`,'success');
    setPForm(f=>({...f,amount:'',matterId:'',payee:'',reference:'',narration:''}));
    setPBalanceCheck(null); setTrustSaving(false); loadTrust();
  }

  async function approvePayment(id){ const {error}=await supabase.from('trust_transactions').update({status:'posted',approved_by:userId,approved_at:new Date().toISOString()}).eq('id',id); if(error){ showTrustAlert('Error: '+error.message,'error'); return; } showTrustAlert('âœ“ Payment approved and posted.','success'); loadTrust(); }
  ...
  async function postTransfer(){
    ...
    if(totalAmount>bal){ showTrustAlert(`âœ— Insufficient balance. Available: ${fmtR(bal)} â€” Required incl. VAT: ${fmtR(totalAmount)}.`,'error'); return; }
    setTrustSaving(true);
    const {error}=await supabase.from('trust_transactions').insert([{
      ...
      narration:tForm.narration||`Transfer of fees â€” ${tForm.matterId}`,
      captured_by:userId,
      branch_id:tForm.branchId||null,
      status:'posted'
    }]);
    if(error){ showTrustAlert('Error: '+error.message,'error'); setTrustSaving(false); return; }
    showTrustAlert(`âœ“ Transfer posted â€” Fees: ${fmtR(amountExclVAT)} + VAT: ${fmtR(vatAmount)} = Total: ${fmtR(totalAmount)} deducted from trust.`,'success');
  ...
  setPwdMsg({msg:'âœ“ Password changed successfully!',type:'success'});
  setPwdSaving(false);
  ...
  async function loadNotes(matterId){
    const{notes}=await fetchMatterNotes(matterId);
    setMatterNotesMap(m=>({...m,[matterId]:notes}));
  }
  ...
  async function handleDeleteNote(noteId,matterId){
    await deleteMatterNote(noteId);
    setMatterNotesMap(m=>({...m,[matterId]:(m[matterId]||[]).filter(n=>n.id!==noteId)}));
  }
  ...
  async function handleEmailInvoice(inv,customEmail){
    ...
    alert(data.warning||`âœ“ Invoice emailed to ${data.sentTo}`);
  }
  ...
  async function logCall(){
    ...
    const durSec=Math.max(6,Number(callForm.durationMins)||6)*60,m=matters.find(x=>x.id===callForm.matterId),title=`ðŸ“ž Call: ${callForm.description}${m?' ['+m.id+']':''}`,now=Date.now(),units=Math.max(1,Math.ceil(durSec/360));
  ...
  async function handleCreateMatter(){
    ...
    setMatterMsg(`Matter ${savedId} created â€” ${toLink.length} activit${toLink.length===1?'y':'ies'} linked.`);
  ...
  function getAnalyticsActs(p){ 
    console.log('allActs total:',allActs.length,'billable:',allActs.filter(a=>a.classification==='billable').length);
    if(p==='day') return allActs.filter(a=>a.date===selDate); if(p==='week'){ const d=new Date(selDate+'T12:00:00'); d.setDate(d.getDate()-d.getDay()+1); const s=d.toISOString().split('T')[0]; const e=new Date(d); e.setDate(d.getDate()+6); return allActs.filter(a=>a.date>=s&&a.date<=e.toISOString().split('T')[0]); } return allActs.filter(a=>a.date.startsWith(selDate.substring(0,7))); }
  ...
  function InvoiceDoc({inv,acts}){
    ...
    return(<div style={{background:'#fff',color:'#111',borderRadius:8,padding:28,fontFamily:'system-ui'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',borderBottom:'2px solid #6CC04A',paddingBottom:16,marginBottom:20}}><div><div style={{fontWeight:900,fontSize:24,letterSpacing:'-0.04em'}}>M<span style={{color:'#6CC04A'}}>B</span></div><div style={{fontSize:11,color:'#999',marginTop:2}}>Motsoeneng Bill</div></div><div style={{textAlign:'right'}}><div style={{fontWeight:800,fontSize:20}}>TAX INVOICE</div><div style={{fontSize:12,color:'#888',marginTop:3}}>{inv.id}</div></div></div>
      ...
      <div style={{marginTop:14,fontSize:10,color:'#ccc',textAlign:'center',lineHeight:1.8}}>Motsoeneng Bill Â· VAT: 4100000000 Â· FNB 62000000000 Â· Branch: 250655<br/>accounts@mb.co.za Â· Computer generated invoice.</div>
    </div>);
  }
  ...
  const TrustTab=useCallback(()=>{
    ...
    return(<div>
      {trustAlert.msg&&(<div style={{background:trustAlert.type==='error'?'rgba(220,80,80,0.1)':'rgba(108,192,74,0.1)',border:`1px solid ${trustAlert.type==='error'?'rgba(220,80,80,0.4)':'rgba(108,192,74,0.3)'}`,borderRadius:6,padding:'10px 14px',marginBottom:14,fontSize:12,color:trustAlert.type==='error'?'#E05252':'#6CC04A',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span>{trustAlert.msg}</span><button style={{background:'none',border:'none',color:'inherit',cursor:'pointer',fontSize:14}} onClick={()=>setTrustAlert({msg:'',type:''})}>âœ•</button></div>)}
      {pendingPayments.length>0&&(<div style={{background:'rgba(234,179,8,0.1)',border:'1px solid rgba(234,179,8,0.3)',borderRadius:6,padding:'10px 14px',marginBottom:14,fontSize:12,color:'#EAB308',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span>â³ {pendingPayments.length} payment{pendingPayments.length>1?'s':''} pending approval â€” {fmtR(pendingPayments.reduce((s,t)=>s+Number(t.amount),0))}</span><button style={{...C.btn('warn'),fontSize:11}} onClick={()=>setTrustTab('approvals')}>Review â†’</button></div>)}
      {balanceAlerts.filter(a=>a.is_active&&getMatterBalance(a.matter_id)<Number(a.minimum_balance)).map(a=>{ const m=matters.find(x=>x.id===a.matter_id); return(<div key={a.matter_id} style={{background:'rgba(220,80,80,0.08)',border:'1px solid rgba(220,80,80,0.3)',borderRadius:6,padding:'8px 14px',marginBottom:8,fontSize:12,color:'#E05252'}}>âš  Low balance: {m?.client||a.matter_id} â€” {fmtR(getMatterBalance(a.matter_id))} below minimum {fmtR(a.minimum_balance)}</div>); })}
      <div style={{display:'flex',gap:6,marginBottom:20,flexWrap:'wrap'}}>
        {[['ledger','ðŸ“Š Ledger'],['receipt','â¬‡ Receipt'],['payment','â¬† Payment'],['transfer','â†” Transfer'],['recon','ðŸ” Reconciliation'],['reports','ðŸ“‹ Reports'],['approvals','âœ… Approvals'],['settings','âš™ Settings']].map(([v,l])=>(
      ...
      {trustTab==='ledger'&&(<div>
        ...
        <div style={C.card}><div style={{fontSize:12,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>All matters â€” trust balances</div><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Matter ID','Client','Description','Branch','Balance','Alert','Actions'].map(h=><th key={h} style={{...C.th,textAlign:h==='Balance'?'right':'left'}}>{h}</th>)}</tr></thead><tbody>{!matters.length&&<tr><td colSpan={7} style={{...C.td,textAlign:'center',color:'#333',padding:30}}>No matters found.</td></tr>}{matters.map(m=>{ const bal=getMatterBalance(m.id),alert=balanceAlerts.find(a=>a.matter_id===m.id&&a.is_active),isLow=alert&&bal<Number(alert.minimum_balance),br=branches.find(b=>b.id===m.branch_id); return(<tr key={m.id} style={{background:isLow?'rgba(220,80,80,0.03)':''}}><td style={{...C.td,fontFamily:'monospace',fontSize:10,color:'#A78BFA'}}>{m.id}</td><td style={{...C.td,fontWeight:500,color:'#C8C8C8'}}>{m.client}</td><td style={{...C.td,color:'#555',fontSize:10}}>{m.name}</td><td style={{...C.td,fontSize:10,color:'#555'}}>{br?.name||'â€”'}</td><td style={{...C.td,fontFamily:'monospace',fontWeight:700,color:bal>0?'#6CC04A':bal<0?'#E05252':'#555',textAlign:'right',whiteSpace:'nowrap'}}>{fmtR(bal)}</td><td style={C.td}>{isLow?<span style={{fontSize:9,color:'#E05252',border:'1px solid rgba(220,80,80,0.4)',padding:'2px 8px',borderRadius:20}}>âš  Low</span>:alert?<span style={{fontSize:9,color:'#555',border:'1px solid #252525',padding:'2px 8px',borderRadius:20}}>Monitored</span>:'â€”'}</td><td style={C.td}><div style={{display:'flex',gap:4}}><button style={{...C.btn('trust'),fontSize:10,padding:'3px 8px'}} onClick={()=>setSelectedTrustMatter(selectedTrustMatter===m.id?'':m.id)}>{selectedTrustMatter===m.id?'Hide':'Ledger'}</button><button style={{...C.btn('g'),fontSize:10,padding:'3px 8px'}} onClick={()=>printTrustStatement(m,getMatterLedger(m.id))}>Statement</button></div></td></tr>); })}<tr style={{background:'#0D0D0D'}}><td colSpan={4} style={{...C.th,paddingTop:12}}>Grand total</td><td style={{...C.th,fontFamily:'monospace',fontSize:12,color:'#6CC04A',textAlign:'right',paddingTop:12}}>{fmtR(total)}</td><td colSpan={2}></td></tr></tbody></table></div></div>
        {selectedTrustMatter&&(<div style={C.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><div style={{fontSize:12,fontWeight:600,color:'#4A90D9'}}>{selectedTrustMatter} â€” {matters.find(m=>m.id===selectedTrustMatter)?.client} â€” Running ledger</div><button style={{...C.btn('g'),fontSize:11}} onClick={()=>printTrustStatement(matters.find(m=>m.id===selectedTrustMatter),ledger)}>Print statement</button></div>{!ledger.length?<div style={{color:'#333',fontSize:12,textAlign:'center',padding:20}}>No transactions yet.</div>:(<div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Date','Type','Receipt No','Reference','Narration','Debit','Credit','Balance'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>{ledger.map((t,i)=>{ const isR=t.type==='receipt'; return(<tr key={i}><td style={{...C.td,fontFamily:'monospace',fontSize:10,whiteSpace:'nowrap'}}>{fmtDate(t.date)}</td><td style={C.td}><span style={{fontSize:9,padding:'2px 8px',borderRadius:20,fontWeight:600,background:isR?'rgba(108,192,74,0.1)':t.type==='payment'?'rgba(220,80,80,0.1)':'rgba(74,144,217,0.1)',color:isR?'#6CC04A':t.type==='payment'?'#E05252':'#4A90D9'}}>{t.type}</span></td><td style={{...C.td,fontFamily:'monospace',fontSize:10,color:'#555'}}>{t.receipt_no||'â€”'}</td><td style={{...C.td,fontSize:10,color:'#555'}}>{t.reference||'â€”'}</td><td style={{...C.td,fontSize:11}}>{t.narration}</td><td style={{...C.td,fontFamily:'monospace',color:'#E05252',textAlign:'right'}}>{!isR?fmtR(t.amount):''}</td><td style={{...C.td,fontFamily:'monospace',color:'#6CC04A',textAlign:'right'}}>{isR?fmtR(t.amount):''}</td><td style={{...C.td,fontFamily:'monospace',fontWeight:700,color:t.runningBalance>=0?'#6CC04A':'#E05252',textAlign:'right'}}>{fmtR(t.runningBalance)}</td></tr>); })}</tbody></table></div>)}</div>)}
      </div>)}
...
{trustTab==='receipt'&&(<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
        <div>
          ...
          {isLocked(rForm.date)&&<div style={{fontSize:10,color:'#E05252',marginTop:4}}>âš  Period locked</div>}
        </div>
        <div>
      ...
      <div>
        ...
        <select style={C.tinp} value={rForm.matterId} onChange={e=>setRForm(f=>({...f,matterId:e.target.value}))}>
          ...
          {allMatters.map(m=><option key={m.id} value={m.id}>{m.id} â€” {m.client}</option>)}
        </select>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
        <div>
  ...
  <div style={C.card}>
    ...
    <div style={{overflowY:'auto',maxHeight:420}}>
      ...
      {trustTab==='payment'&&(<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <div style={C.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0'}}>New trust payment</div><span style={{fontSize:10,color:'#EAB308',border:'1px solid rgba(234,179,8,0.3)',padding:'2px 10px',borderRadius:20}}>â‰¥ {fmtR(APPROVAL_THRESHOLD)} needs approval</span></div>
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><div><label style={C.lbl}>Date *</label><input type="date" style={C.tinp} value={pForm.date} onChange={e=>setPForm(f=>({...f,date:e.target.value}))}/>{isLocked(pForm.date)&&<div style={{fontSize:10,color:'#E05252',marginTop:4}}>âš  Period locked</div>}</div><div><label style={C.lbl}>Amount (ZAR) *</label><input type="text" inputMode="decimal" style={C.tinp} placeholder="0.00" defaultValue={pForm.amount} onBlur={e=>{ const v=e.target.value.replace(/[^0-9.]/g,''); e.target.value=v; setPForm(f=>({...f,amount:v})); checkPaymentBalance(pForm.matterId,v); }}/></div></div>
            <div><label style={C.lbl}>Matter *</label><select style={C.tinp} value={pForm.matterId} onChange={e=>{ setPForm(f=>({...f,matterId:e.target.value})); checkPaymentBalance(e.target.value,pForm.amount); }}><option value="">Select matter...</option>{matters.map(m=><option key={m.id} value={m.id}>{m.id} â€” {m.client} (bal: {fmtR(getMatterBalance(m.id))})</option>)}</select></div>
            {pBalanceCheck&&(<div style={{background:pBalanceCheck.ok?'rgba(108,192,74,0.08)':'rgba(220,80,80,0.08)',border:`1px solid ${pBalanceCheck.ok?'rgba(108,192,74,0.3)':'rgba(220,80,80,0.3)'}`,borderRadius:6,padding:'8px 12px',fontSize:12,color:pBalanceCheck.ok?'#6CC04A':'#E05252'}}>{pBalanceCheck.ok?`âœ“ Available: ${fmtR(pBalanceCheck.bal)} Â· After: ${fmtR(pBalanceCheck.bal-pBalanceCheck.amt)}${pBalanceCheck.needsApproval?' Â· â³ Needs partner approval':''}`:`âœ— Insufficient â€” available: ${fmtR(pBalanceCheck.bal)}, shortfall: ${fmtR(pBalanceCheck.amt-pBalanceCheck.bal)}`}</div>)}
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><div><label style={C.lbl}>Trust bank account</label><select style={C.tinp} value={pForm.accountId} onChange={e=>setPForm(f=>({...f,accountId:e.target.value}))}>{trustAccounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></div><div><label style={C.lbl}>Branch</label><div style={{...C.tinp,color:'#4A90D9',display:'flex',alignItems:'center'}} onClick={()=>setPForm(f=>({...f,branchId:profile?.branch_id||''}))}>{branches.find(b=>b.id===profile?.branch_id)?.name||'Not assigned'}</div></div></div>
      ...
      {trustTab==='transfer'&&(<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <div style={C.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0'}}>Trust to business transfer</div><span style={{fontSize:10,color:'#4A90D9',border:'1px solid rgba(74,144,217,0.3)',padding:'2px 10px',borderRadius:20}}>Both legs auto-posted</span></div>
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><div><label style={C.lbl}>Date *</label><input type="date" style={C.tinp} value={tForm.date} onChange={e=>setTForm(f=>({...f,date:e.target.value}))}/>{isLocked(tForm.date)&&<div style={{fontSize:10,color:'#E05252',marginTop:4}}>âš  Period locked</div>}</div><div><label style={C.lbl}>Amount (ZAR) *</label><input type="text" inputMode="decimal" style={C.tinp} placeholder="0.00" defaultValue={tForm.amount} onBlur={e=>{ const v=e.target.value.replace(/[^0-9.]/g,''); e.target.value=v; setTForm(f=>({...f,amount:v})); checkTransferBalance(tForm.matterId,v); }}/></div></div>
            <div><label style={C.lbl}>Matter *</label><select style={C.tinp} value={tForm.matterId} onChange={e=>{ setTForm(f=>({...f,matterId:e.target.value,invoiceId:'',amount:''})); checkTransferBalance(e.target.value,tForm.amount); }}><option value="">Select matter...</option>{matters.map(m=><option key={m.id} value={m.id}>{m.id} â€” {m.client} (bal: {fmtR(getMatterBalance(m.id))})</option>)}</select></div>
  ...
  {tBalanceCheck.ok?(
    <div>
      <div>âœ“ Available: {fmtR(tBalanceCheck.bal)}</div>
  ...
  ):`âœ— Insufficient â€” available: ${fmtR(tBalanceCheck.bal)}, required incl. VAT: ${fmtR(tBalanceCheck.amt*1.15)}`}
</div>)}
            {tForm.matterId&&getMatterInvoices(tForm.matterId).length>0&&(<div><label style={C.lbl}>Link to invoice (auto-fills amount)</label><select style={C.tinp} value={tForm.invoiceId} onChange={e=>{ const inv=invoices.find(i=>i.id===e.target.value); setTForm(f=>({...f,invoiceId:e.target.value,narration:inv?`Transfer of fees â€” ${inv.matter_name} â€” ${inv.id}`:''})); }}><option value="">Select invoice (optional)...</option>{getMatterInvoices(tForm.matterId).map(i=><option key={i.id} value={i.id}>{i.id} â€” R{((i.total_units||0)*(i.rate||150)*1.15).toFixed(2)} incl. VAT</option>)}</select><div style={{fontSize:10,color:'#555',marginTop:4}}>Linking an invoice is optional â€” enter amount manually</div></div>)}
      ...
      {trustTab==='reports'&&(<div>
        ...
        {reportType==='trial'&&(<div style={C.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0'}}>Trust trial balance{reportBranch?' â€” '+branches.find(b=>b.id===reportBranch)?.name:' â€” all branches'}</div><span style={{fontSize:10,color:'#555'}}>Grand total must equal trust bank balance</span></div><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Matter ID','Client','Description','Branch','Trust Balance'].map(h=><th key={h} style={{...C.th,textAlign:h==='Trust Balance'?'right':'left'}}>{h}</th>)}</tr></thead><tbody>{matters.map(m=>{ const bal=getMatterBalance(m.id),br=branches.find(b=>b.id===m.branch_id); return(<tr key={m.id} style={{opacity:bal===0?0.4:1}}><td style={{...C.td,fontFamily:'monospace',fontSize:10,color:'#A78BFA'}}>{m.id}</td><td style={{...C.td,fontWeight:500}}>{m.client}</td><td style={{...C.td,fontSize:10,color:'#555'}}>{m.name}</td><td style={{...C.td,fontSize:10,color:'#555'}}>{br?.name||'â€”'}</td><td style={{...C.td,fontFamily:'monospace',fontWeight:700,textAlign:'right',color:bal>0?'#6CC04A':bal<0?'#E05252':'#555'}}>{fmtR(bal)}</td></tr>); })}<tr style={{background:'#0D0D0D'}}><td colSpan={4} style={{...C.th,paddingTop:12}}>Grand total</td><td style={{...C.th,fontFamily:'monospace',fontSize:12,color:'#6CC04A',textAlign:'right',paddingTop:12}}>{fmtR(totalTrustHeld())}</td></tr></tbody></table></div>)}
        {reportType==='receipts'&&(<div style={C.card}><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>Receipts journal â€” {reportFrom} to {reportTo}</div><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Receipt No','Date','Matter','Client','Received From','Branch','Narration','Amount'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>{getReportTxns().filter(t=>t.type==='receipt').map((t,i)=>{ const m=matters.find(x=>x.id===t.matter_id),br=branches.find(b=>b.id===t.branch_id); return(<tr key={i}><td style={{...C.td,fontFamily:'monospace',fontSize:10,color:'#4A90D9'}}>{t.receipt_no}</td><td style={{...C.td,fontSize:10}}>{fmtDate(t.date)}</td><td style={{...C.td,fontSize:10,color:'#A78BFA'}}>{t.matter_id}</td><td style={C.td}>{m?.client||'â€”'}</td><td style={{...C.td,fontSize:10,color:'#555'}}>{t.received_from||'â€”'}</td><td style={{...C.td,fontSize:10,color:'#555'}}>{br?.name||'â€”'}</td><td style={{...C.td,fontSize:10,color:'#555'}}>{t.narration}</td><td style={{...C.td,fontFamily:'monospace',color:'#6CC04A',textAlign:'right'}}>{fmtR(t.amount)}</td></tr>); })}<tr style={{background:'#0D0D0D'}}><td colSpan={7} style={{...C.th,paddingTop:12}}>Total</td><td style={{...C.th,fontFamily:'monospace',fontSize:12,color:'#6CC04A',textAlign:'right',paddingTop:12}}>{fmtR(getReportTxns().filter(t=>t.type==='receipt').reduce((s,t)=>s+Number(t.amount),0))}</td></tr></tbody></table></div>)}
        {reportType==='payments'&&(<div style={C.card}><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>Payments journal â€” {reportFrom} to {reportTo}</div><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Date','Matter','Client','Payee','Branch','Status','Narration','Amount'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>{getReportTxns().filter(t=>t.type==='payment').map((t,i)=>{ const m=matters.find(x=>x.id===t.matter_id),br=branches.find(b=>b.id===t.branch_id); return(<tr key={i}><td style={{...C.td,fontSize:10}}>{fmtDate(t.date)}</td><td style={{...C.td,fontSize:10,color:'#A78BFA'}}>{t.matter_id}</td><td style={C.td}>{m?.client||'â€”'}</td><td style={C.td}>{t.payee}</td><td style={{...C.td,fontSize:10,color:'#555'}}>{br?.name||'â€”'}</td><td style={C.td}><span style={{fontSize:9,padding:'2px 8px',borderRadius:20,fontWeight:600,background:t.status==='posted'?'rgba(108,192,74,0.1)':'rgba(234,179,8,0.1)',color:t.status==='posted'?'#6CC04A':'#EAB308'}}>{t.status}</span></td><td style={{...C.td,fontSize:10,color:'#555'}}>{t.narration}</td><td style={{...C.td,fontFamily:'monospace',color:'#E05252',textAlign:'right'}}>{fmtR(t.amount)}</td></tr>); })}<tr style={{background:'#0D0D0D'}}><td colSpan={7} style={{...C.th,paddingTop:12}}>Total</td><td style={{...C.th,fontFamily:'monospace',fontSize:12,color:'#E05252',textAlign:'right',paddingTop:12}}>{fmtR(getReportTxns().filter(t=>t.type==='payment').reduce((s,t)=>s+Number(t.amount),0))}</td></tr></tbody></table></div>)}
        {reportType==='transfers'&&(<div style={C.card}><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>Transfers journal â€” {reportFrom} to {reportTo}</div><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Date','Matter','Client','From','To Business','Invoice','Amount'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>{getReportTxns().filter(t=>t.type==='transfer').map((t,i)=>{ const m=matters.find(x=>x.id===t.matter_id); return(<tr key={i}><td style={{...C.td,fontSize:10}}>{fmtDate(t.date)}</td><td style={{...C.td,fontSize:10,color:'#A78BFA'}}>{t.matter_id}</td><td style={C.td}>{m?.client||'â€”'}</td><td style={{...C.td,fontSize:10,color:'#555'}}>{trustAccounts.find(a=>a.id===t.trust_account_id)?.name||'Trust'}</td><td style={{...C.td,fontSize:10,color:'#6CC04A'}}>{t.to_account}</td><td style={{...C.td,fontSize:10,color:'#555'}}>{t.invoice_id||'â€”'}</td><td style={{...C.td,fontFamily:'monospace',color:'#4A90D9',textAlign:'right'}}>{fmtR(t.amount)}</td></tr>); })}<tr style={{background:'#0D0D0D'}}><td colSpan={6} style={{...C.th,paddingTop:12}}>Total</td><td style={{...C.th,fontFamily:'monospace',fontSize:12,color:'#4A90D9',textAlign:'right',paddingTop:12}}>{fmtR(getReportTxns().filter(t=>t.type==='transfer').reduce((s,t)=>s+Number(t.amount),0))}</td></tr></tbody></table></div>)}
      </div>)}
      ...
      {trustTab==='settings'&&(<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <div style={C.card}><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0',marginBottom:16}}>Low balance alerts</div>
          <div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:16}}>
            <div><label style={C.lbl}>Matter</label><select style={C.tinp} value={alertMatterId} onChange={e=>setAlertMatterId(e.target.value)}><option value="">Select matter...</option>{allMatters.map(m=><option key={m.id} value={m.id}>{m.id} â€” {m.client}</option>)}</select></div>
  ...
  // â”€â”€ Target gauge calculations â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return(<>
    ...
    <div style={C.page}>
      <NavBar
        ...
        rightSlot={<>
          <div style={{position:'relative'}}><input ref={searchRef} style={{background:'#1A1A1A',border:`1px solid ${searchQuery?'rgba(108,192,74,0.4)':'#252525'}`,color:'#F0F0F0',padding:'5px 12px 5px 32px',borderRadius:20,fontSize:12,fontFamily:'inherit',width:180,outline:'none'}} placeholder="Search..." value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}/><span style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',fontSize:13,pointerEvents:'none'}}>{searching?'âŒ›':'ðŸ”'}</span>{searchQuery&&<button style={{position:'absolute',right:8,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',color:'#666',cursor:'pointer',fontSize:12,padding:0}} onClick={()=>setSearchQuery('')}>âœ•</button>}</div>
          <button style={{background:'transparent',border:'1px solid #252525',color:'#555',padding:'4px 10px',borderRadius:6,cursor:'pointer',fontSize:11,fontFamily:'inherit'}} onClick={()=>setShowPwdForm(true)}>ðŸ”’</button>
          {!online&&<span style={{fontSize:11,color:'#3A3A3A'}}>Offline</span>}
        </>}
      />

      {/* â”€â”€ TARGET GAUGE â€” always visible â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {monthTarget>0&&(<div style={{background:'#0D0D0D',borderBottom:'1px solid #1A1A1A',padding:'8px 24px',display:'flex',alignItems:'center',gap:16,flexWrap:'wrap'}}>
        ...
        {monthPct!==null&&monthPct>=100&&<div style={{fontSize:10,color:'#6CC04A',fontWeight:700}}>ðŸŽ‰ Target hit!</div>}
      </div>)}

      {tab==='today'&&(<div style={C.main}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}><div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>{fdate(selDate)}</div><div style={{fontSize:11,color:'#444'}}>{dayActs.length} sessions Â· {toHm(daySec)} total</div></div><div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}><select style={C.sel} value={selDate} onChange={e=>setSelDate(e.target.value)}><option value={today}>Today</option>{dates.filter(d=>d.date!==today).map(d=><option key={d.date} value={d.date}>{fdate(d.date)} ({d.sessions})</option>)}</select><button style={C.btn('pur')} onClick={()=>setShowCall(true)}>ðŸ“ž Log a Call</button><button style={C.btn('p')} onClick={()=>setTab('invoices')}>Generate Invoice</button></div></div>
        {!liveActs.length&&<div style={{background:'rgba(74,144,217,0.05)',border:'1px solid rgba(74,144,217,0.15)',borderRadius:6,padding:'10px 14px',marginBottom:14,fontSize:11,color:'#666'}}><strong style={{color:'#4A90D9'}}>No live data yet</strong> â€” Electron agent must be running.</div>}
        ...
        {(()=>{
          ...
          return(<>
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:14}}>
              {[{l:'Total',v:undertakings.length},{l:'Pending',v:pending.length,w:pending.length>0},{l:'Overdue',v:overdue.length,w:overdue.length>0}].map(({l,v,w})=>(<div key={l} style={C.stat(false,w)}><div style={{fontSize:9,color:'#555',textTransform:'uppercase',letterSpacing:'.09em',marginBottom:8}}>{l}</div><div style={{fontSize:22,fontWeight:800,color:w&&v>0?'#EAB308':'#F0F0F0'}}>{v}</div></div>))}
            </div>
            {!undertakings.length?(<div style={{...C.card,textAlign:'center',padding:40,color:'#555'}}><div style={{fontSize:28,marginBottom:10}}>ðŸ¤</div><div>No undertakings yet</div></div>):
            <div style={C.card}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Matter','Direction','Description','Given To','Due Date','Status','Action'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
              {undertakings.map(u=>{const isOD=u.due_date&&new Date(u.due_date)<new Date()&&u.status==='pending';return(<tr key={u.id}>
                ...
                <td style={{...C.td,color:'#777',fontSize:10}}>{u.given_to||'â€”'}</td>
                <td style={{...C.td,fontFamily:'monospace',color:isOD?'#E05252':'#888',fontWeight:isOD?700:400}}>{u.due_date?fmtDate(u.due_date):'â€”'}{isOD&&' âš '}</td>
                ...
                <td style={C.td}><div style={{display:'flex',gap:4}}>{u.status==='pending'&&<button style={{...C.btn('p'),fontSize:10,padding:'3px 8px'}} onClick={async()=>{await fulfillUndertaking(u.id);fetchUndertakings({userId}).then(r=>setUndertakings(r.undertakings||[])); }}>âœ“</button>}<button style={{...C.btn('r'),fontSize:10,padding:'3px 8px'}} onClick={async()=>{if(!confirm('Delete?'))return;await deleteUndertaking(u.id);fetchUndertakings({userId}).then(r=>setUndertakings(r.undertakings||[]));}}>âœ•</button></div></td>
              </tr>);})}
            </tbody></table></div>}
          </>);
        })()}
        {showUTForm&&(<div style={C.modal} onClick={()=>setShowUTForm(false)}><div style={{...C.mbox,maxWidth:500}} onClick={e=>e.stopPropagation()}>
          <div style={{fontSize:15,fontWeight:700,marginBottom:16}}>New Undertaking</div>
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              <div><label style={C.lbl}>Matter *</label><select style={C.inp} value={utForm.matter_id} onChange={e=>setUtForm(f=>({...f,matter_id:e.target.value}))}><option value="">â€” Select â€”</option>{matters.map(m=><option key={m.id} value={m.id}>{m.id} Â· {m.client}</option>)}</select></div>
      ...
      {tab==='communications'&&(<div style={C.main}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
          <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Client Communications</div><div style={{fontSize:11,color:'#444'}}>Log of all client interactions on your matters</div></div>
          <button style={C.btn('p')} onClick={()=>{setCommForm({client_id:'',matter_id:'',comm_type:'call',direction:'outbound',subject:'',body:'',comm_date:today});setShowCommForm(true);}}>+ Log Communication</button>
        </div>
        {!communications.length?(<div style={{...C.card,textAlign:'center',padding:40,color:'#555'}}><div style={{fontSize:28,marginBottom:10}}>ðŸ’¬</div><div>No communications logged yet</div></div>):
        <div style={C.card}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Date','Type','Direction','Matter','Subject / Notes','Action'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
          {communications.map(c=>{const typeCol={call:'#4A90D9',email:'#6CC04A',meeting:'#A78BFA',letter:'#EAB308',note:'#888',whatsapp:'#25D366'};return(<tr key={c.id}>
            <td style={{...C.td,fontSize:10,color:'#666'}}>{fmtDate(c.comm_date)}</td>
            <td style={C.td}><span style={{fontSize:9,padding:'1px 8px',borderRadius:20,background:'rgba(74,144,217,0.08)',color:typeCol[c.comm_type]||'#888',fontWeight:600,textTransform:'capitalize'}}>{c.comm_type}</span></td>
            ...
            <td style={{...C.td,fontFamily:'monospace',color:'#A78BFA',fontSize:10}}>{c.matter_id||'â€”'}</td>
            <td style={{...C.td,color:'#888',maxWidth:200}}>{c.subject||c.body.substring(0,50)}</td>
            <td style={C.td}><button style={{...C.btn('r'),fontSize:10,padding:'3px 8px'}} onClick={async()=>{if(!confirm('Delete?'))return;await deleteClientCommunication(c.id);fetchClientCommunications({userId}).then(r=>setCommunications(r.communications||[])); }}>âœ•</button></td>
          </tr>);})}
        </tbody></table></div>}
        {showCommForm&&(<div style={C.modal} onClick={()=>setShowCommForm(false)}><div style={{...C.mbox,maxWidth:520}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            ...
            <div><label style={C.lbl}>Matter</label><select style={C.inp} value={commForm.matter_id} onChange={e=>setCommForm(f=>({...f,matter_id:e.target.value}))}><option value="">â€” Optional â€”</option>{matters.map(m=><option key={m.id} value={m.id}>{m.id} Â· {m.client}</option>)}</select></div>
  ...
  {(()=>{
    const cats={copies:'ðŸ“‹',filing_fee:'ðŸ“',sheriff:'âš–ï¸',travel:'ðŸš—',counsel:'ðŸ‘”',search:'ðŸ”',postage:'ðŸ“®',other:'ðŸ“Ž'};
        ...
        <table style={{width:'100%',borderCollapse:'collapse'}}>
          ...
          <tbody>
            ...
            {disbursements.map(d=>{const m=matters.find(x=>x.id===d.matter_id);return(<tr key={d.id}>
              <td style={{padding:'9px 10px',fontSize:16,borderBottom:'1px solid #161616'}}>{cats[d.category]||'ðŸ“Ž'}</td>
              <td style={{padding:'9px 10px',fontSize:11,borderBottom:'1px solid #161616',color:'#666'}}>{d.date}</td>
              <td style={{padding:'9px 10px',fontSize:10,borderBottom:'1px solid #161616',fontFamily:'monospace',color:'#A78BFA'}}>{d.matter_id||'â€”'}{m&&<div style={{fontSize:9,color:'#555'}}>{m.client}</div>}</td>
  ...
  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10}}>
    <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Analytics</div><div style={{fontSize:11,color:'#444'}}>{analyticsPeriod==='day'?fdate(selDate):analyticsPeriod==='week'?'This week':fmonth(selDate)} Â· {getAnalyticsActs(analyticsPeriod).length} sessions</div></div>
    <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
      ...
      <div style={{display:'flex',background:'#1A1A1A',border:'1px solid #252525',borderRadius:6,padding:2}}>
        {[['day','Day'],['week','Week'],['month','Month']].map(([v,l])=>(<button key={v} style={C.ptab(analyticsPeriod===v)} onClick={()=>setAP(v)}>{l}</button>))}
      </div>
    </div>
  </div>
  {(()=>{
          ...
          return allYears.map(yr=>{
            ...
            return(<div key={yr} style={{marginBottom:28}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                ...
                <div style={{fontSize:11,color:'#555'}}>{yrTotal} units Â· R{yrRev.toLocaleString(undefined,{maximumFractionDigits:0})} revenue</div>
              </div>
          ...
          const halfSummary=(months,label)=>{
            ...
            return(<div key={label} style={{...C.card,background:'rgba(108,192,74,0.03)',border:'1px solid rgba(108,192,74,0.15)'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12,flexWrap:'wrap',gap:8}}>
                <div style={{fontSize:13,fontWeight:700,color:'#D0D0D0'}}>{label} Performance Review â€” {yr}</div>
...
\nBillable Time: ${toHm2(sec)}\nUtilisation: ${util}%\nRevenue Invoiced: R${invAmt.toFixed(2)} incl. VAT\n\nMonthly Breakdown:\n${months.map(m=>{const u=allActs.filter(a=>a.is_billable&&a.date?.startsWith(m)).reduce((s,a)=>s+(a.billing_units||0),0);return new Date(m+'-01T12:00:00').toLocaleString('en-ZA',{month:'long'})+': '+u+' units';}).join('\n')}`;
                  navigator.clipboard.writeText(txt);alert('âœ“ Performance summary copied â€” paste into your review document');
                }}>ðŸ“‹ Copy for Review</button>
              </div>
          ...
          return(<div style={{marginTop:20}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
              {halfSummary(h1months,'H1 (Janâ€“Jun')}
              {halfSummary(h2months,'H2 (Julâ€“Dec')}
            </div>
          </div>);
        })()}
      </div>)}

      {tab==='trust'&&(<div style={C.main}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10}}><div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>ðŸ¦ Trust Accounting</div><div style={{fontSize:11,color:'#444'}}>Legal Practice Act compliant Â· 3 branches Â· balance never goes negative Â· payments â‰¥ {fmtR(APPROVAL_THRESHOLD)} require approval</div></div><button style={C.btn()} onClick={loadTrust}>â†» Refresh</button></div><TrustTab/></div>)}
    </div>

    {showCall&&(<div style={C.modal} onClick={()=>setShowCall(false)}><div style={C.mbox} onClick={e=>e.stopPropagation()}><div style={{fontSize:15,fontWeight:700,marginBottom:4}}>ðŸ“ž Log a Call</div><div style={{fontSize:11,color:'#555',marginBottom:20}}>Record a client call as a billable activity</div><div style={{display:'flex',flexDirection:'column',gap:12}}><div><label style={C.lbl}>Description *</label><input style={C.inp} placeholder="e.g. Smith â€” settlement discussion" value={callForm.description} onChange={e=>setCallForm(f=>({...f,description:e.target.value}))}/></div><div><label style={C.lbl}>Assign to matter</label><select style={C.inp} value={callForm.matterId} onChange={e=>setCallForm(f=>({...f,matterId:e.target.value}))}><option value="">â€” optional â€”</option>{matters.map(m=><option key={m.id} value={m.id}>{m.id} Â· {m.name} ({m.client})</option>)}</select></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><div><label style={C.lbl}>Duration (minutes)</label><input style={C.inp} type="number" min="1" value={callForm.durationMins} onChange={e=>setCallForm(f=>({...f,durationMins:parseInt(e.target.value)||6}))}/><div style={{fontSize:10,color:'#6CC04A',marginTop:4}}>{calcUnits(callForm.durationMins*60)} unit(s) Â· R{calcAmt(callForm.durationMins*60,invRate).toLocaleString()}</div></div><div><label style={C.lbl}>Logged at</label><div style={{...C.inp,display:'flex',alignItems:'center',color:'#888',fontSize:11}}>{new Date().toLocaleTimeString('en-ZA',{hour:'2-digit',minute:'2-digit'})}</div></div></div></div><div style={{display:'flex',gap:10,marginTop:20,justifyContent:'flex-end'}}><button style={C.btn()} onClick={()=>setShowCall(false)}>Cancel</button><button style={C.btn('pur')} onClick={logCall} disabled={callSaving||!callForm.description}>{callSaving?'Saving...':'Save as Billable'}</button></div></div></div>)}

    {showCNForm&&cnInvoice&&(<div style={C.modal} onClick={()=>setShowCNForm(false)}><div style={{...C.mbox,maxWidth:420}} onClick={e=>e.stopPropagation()}><div style={{fontSize:15,fontWeight:700,marginBottom:4}}>Issue Credit Note</div><div style={{fontSize:11,color:'#555',marginBottom:16}}>Against invoice <strong style={{color:'#F0F0F0'}}>{cnInvoice.id}</strong> Â· {cnInvoice.client} Â· R{((cnInvoice.total_units||0)*(cnInvoice.rate||150)*1.15).toFixed(2)} incl. VAT</div><div style={{display:'flex',flexDirection:'column',gap:12}}><div><label style={C.lbl}>Credit amount (R) *</label><input style={C.inp} type="number" placeholder="0.00" value={cnForm.amount} onChange={e=>setCnForm(f=>({...f,amount:e.target.value}))}/></div><div><label style={C.lbl}>Reason *</label><textarea style={{...C.inp,minHeight:70,resize:'vertical'}} placeholder="Reason for credit note..." value={cnForm.reason} onChange={e=>setCnForm(f=>({...f,reason:e.target.value}))}/></div></div><div style={{display:'flex',gap:8,marginTop:16,justifyContent:'flex-end'}}><button style={C.btn()} onClick={()=>setShowCNForm(false)}>Cancel</button><button style={C.btn('p')} disabled={savingCN||!cnForm.amount||!cnForm.reason.trim()} onClick={handleSaveCreditNote}>{savingCN?'Savingâ€¦':'Issue Credit Note'}</button></div></div></div>)}

    {showMatterForm&&(<div style={C.modal} onClick={()=>setShowMatterForm(false)}><div style={C.mbox} onClick={e=>e.stopPropagation()}><div style={{fontSize:15,fontWeight:700,marginBottom:4}}>ðŸ“ New Client Matter</div><div style={{fontSize:11,color:'#555',marginBottom:20}}>Activities auto-linked by matching window titles.</div><div style={{display:'flex',flexDirection:'column',gap:12}}><div><label style={C.lbl}>Matter ID *</label><input style={C.inp} placeholder="e.g. L2025/042" value={matterForm.id} onChange={e=>setMatterForm(f=>({...f,id:e.target.value.toUpperCase()}))}/></div><div><label style={C.lbl}>Matter name *</label><input style={C.inp} placeholder="e.g. Smith v Jones â€” Contract Review" value={matterForm.name} onChange={e=>setMatterForm(f=>({...f,name:e.target.value}))}/></div><div><label style={C.lbl}>Client name *</label><input style={C.inp} placeholder="e.g. ABC Corporation" value={matterForm.client} onChange={e=>setMatterForm(f=>({...f,client:e.target.value}))}/></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}><div><label style={C.lbl}>Prescription Date</label><input style={C.inp} type="date" value={matterForm.prescription_date||''} onChange={e=>setMatterForm(f=>({...f,prescription_date:e.target.value}))}/></div><div><label style={C.lbl}>Next Action Date</label><input style={C.inp} type="date" value={matterForm.next_action_date||''} onChange={e=>setMatterForm(f=>({...f,next_action_date:e.target.value}))}/></div><div><label style={C.lbl}>Budget (units)</label><input style={C.inp} type="number" min="0" placeholder="0" value={matterForm.budget_units||''} onChange={e=>setMatterForm(f=>({...f,budget_units:parseInt(e.target.value)||0}))}/></div></div><div><label style={C.lbl}>Description</label><input style={C.inp} placeholder="Optional" value={matterForm.description} onChange={e=>setMatterForm(f=>({...f,description:e.target.value}))}/></div></div><div style={{display:'flex',gap:10,marginTop:20,justifyContent:'flex-end'}}><button style={C.btn()} onClick={()=>setShowMatterForm(false)}>Cancel</button><button style={C.btn('p')} onClick={handleCreateMatter} disabled={matterSaving||!matterForm.id||!matterForm.name||!matterForm.client}>{matterSaving?'Creating...':'Create Matter'}</button></div></div></div>)}
  ...
  <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowPwdForm(false)}>
    <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:32,width:'100%',maxWidth:400}} onClick={e=>e.stopPropagation()}>
      <div style={{fontSize:15,fontWeight:700,color:'#F0F0F0',marginBottom:4}}>ðŸ”’ Change Password</div>
