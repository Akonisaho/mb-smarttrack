...
function printTrustStatement(matter, transactions) {
  ...
  <div class="top"><div><div class="logo">M<span>B</span></div><div style="font-size:11px;color:#999;margin-top:2px">Motsoeneng Bill</div></div><div style="text-align:right"><h2>TRUST ACCOUNT STATEMENT</h2><div style="font-size:11px;color:#999">Matter: ${matter?.id} · Generated: ${fdate(new Date().toISOString().split('T')[0])}</div></div></div>
...
// downloadPDF is now handled inside App via generateDetailedInvoicePDF (firm context needed)
// keeping a shim here so any remaining static refs don't break during build
...
const [saForm,setSaForm]=useState({billing_score:0,client_score:0,teamwork_score:0,knowledge_score:0,overall_score:0,comments:''});
...
const [prReviewId,setPrReviewId]=useState('');
...
const rFormDirty=useRef(false);
  ...
  useEffect(()=>{
    ...
    const loadPf=async()=>{
      const [{feedback:rcvd},{feedback:sent}]=await Promise.all([
        fetchPerformanceFeedback({toUserId:userId}),
        fetchPerformanceFeedback({fromUserId:userId}),
      ]);
      const combined=[...(rcvd||[]),...(sent||[])].sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));
      setPerfFeedback(combined);
    };
    loadPf();
  },[authLoading,userId]);
  ...
  useEffect(()=>{
    ...
    fetchPerformanceReviews().then(async({reviews})=>{
      ...
      // Peer review — which colleagues already reviewed this period?
  ...
  useEffect(()=>{
    ...
    supabase.from('profiles')
      ...
      .neq('id',userId)
      .in('role',['attorney','branch_manager','manager','national_manager'])
      .then(({data})=>setBranchColleagues(data||[]));
  },[profile?.branch_id,userId]);

  useEffect(()=>{
    if(!showFbPanel||!userId||!perfFeedback.length) return;
    const unread=perfFeedback.filter(f=>!f.is_read&&f.from_user_id!==userId).map(f=>f.id);
    if(!unread.length) return;
    Promise.all(unread.map(id=>markFeedbackRead(id))).then(()=>{
...
{trustTab==='receipt'&&(<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
    ...
    <div style={C.page}>
      ...
      {tab==='today'&&(<div style={C.main}>
          ...
          <div key={rv.id} style={{background:'rgba(141,198,63,0.04)',border:'1px solid rgba(141,198,63,0.3)',borderRadius:8,padding:14,marginBottom:14,display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
            <div>
              ...
              <div style={{fontSize:11,color:'#555',marginTop:3}}>
                {rv.due_date?`Due ${new Date(rv.due_date+'T12:00:00').toLocaleDateString('en-ZA',{weekday:'short',day:'numeric',month:'long'})} · `:''}
                {rv.instructions||'Please complete your performance self-assessment.'}
              </div>
            </div>
            <button style={{...C.btn('p'),fontSize:12,padding:'7px 18px',whiteSpace:'nowrap'}} onClick={()=>{setSaForm({billing_score:0,client_score:0,teamwork_score:0,knowledge_score:0,overall_score:0,comments:'',reviewId:rv.id,period:rv.period});setShowSelfAssess(true);}}>
        ...
        {openReviews.map(rv=>{
          ...
          return(<div key={'pr-'+rv.id} style={{background:'rgba(74,144,217,0.04)',border:'1px solid rgba(74,144,217,0.28)',borderRadius:8,padding:14,marginBottom:14,display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
            <div>
              <div style={{fontSize:13,fontWeight:700,color:'#4A90D9'}}>👥 Peer Reviews Due — {rv.period}</div>
              <div style={{fontSize:11,color:'#555',marginTop:3}}>
                {pending.length} colleague{pending.length!==1?'s':''} to review
                {rv.due_date?` · Due ${new Date(rv.due_date+'T12:00:00').toLocaleDateString('en-ZA',{weekday:'short',day:'numeric',month:'long'})}`:''}
              </div>
            </div>
            <button style={{background:'#4A90D9',border:'none',color:'#fff',padding:'7px 18px',borderRadius:6,cursor:'pointer',fontSize:12,fontFamily:'inherit',fontWeight:700,whiteSpace:'nowrap'}}
              onClick={()=>{setPrPeriod(rv.period);setPrReviewId(rv.id);setPrForm({billing_score:0,client_score:0,teamwork_score:0,knowledge_score:0,overall_score:0,comments:'',is_anonymous:false});setShowPeerReview(true);}}>
              Start Peer Reviews ({pending.length})
            </button>
          </div>);
        })}
        {openReviews.map(rv=>{
          const allDone=branchColleagues.length>0&&branchColleagues.every(c=>peerReviewedIds.includes(c.id));
          if(!allDone||!branchColleagues.length) return null;
          return(<div key={'prdone-'+rv.id} style={{background:'rgba(74,144,217,0.03)',border:'1px solid rgba(74,144,217,0.15)',borderRadius:8,padding:'10px 14px',marginBottom:14,display:'flex',alignItems:'center',gap:8}}>
        ...
        {perfFeedback.length>0&&(<div style={{background:'rgba(244,114,182,0.04)',border:`1px solid ${perfFeedback.filter(f=>!f.is_read&&f.from_user_id!==userId).length>0?'rgba(244,114,182,0.35)':'rgba(244,114,182,0.12)'}`,borderRadius:8,padding:14,marginBottom:14}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:showFbPanel?10:0}}>
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              ...
              {perfFeedback.filter(f=>!f.is_read&&f.from_user_id!==userId).length>0&&<span style={{fontSize:10,padding:'2px 8px',borderRadius:20,background:'rgba(244,114,182,0.15)',color:'#F472B6',fontWeight:700}}>{perfFeedback.filter(f=>!f.is_read&&f.from_user_id!==userId).length} new</span>}
            </div>
            <button style={{...C.btn(),fontSize:11}} onClick={()=>setShowFbPanel(s=>!s)}>{showFbPanel?'▲ Hide':'▼ Open'}</button>
          </div>
          {showFbPanel&&(<div>
      ...
      {showSelfAssess&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20,overflowY:'auto'}} onClick={()=>setShowSelfAssess(false)}>
            ...
            {[['billing_score','Billing & Target Achievement'],['client_score','Client Service & Communication'],['teamwork_score','Teamwork & Professional Attitude'],['knowledge_score','Legal Knowledge & Expertise'],['overall_score','Overall Self-Rating']].map(([key,label])=>(
              <div key={key}>
                ...
                <div style={{display:'flex',gap:6}}>
                  {[1,2,3,4,5].map(n=>(
                    <button key={n} style={{flex:1,padding:'8px 4px',borderRadius:6,border:`1px solid ${saForm[key]===n?'#6CC04A':'#252525'}`,background:saForm[key]===n?'rgba(108,192,74,0.12)':'#1A1A1A',color:saForm[key]===n?'#6CC04A':'#555',cursor:'pointer',fontSize:13,fontFamily:'inherit',fontWeight:saForm[key]===n?700:400}} onClick={()=>setSaForm(f=>({...f,[key]:n}))}>
                      {n} {'★'.repeat(n)}
                    </button>
                  ))}
                </div>
      ...
      {/* ── PEER REVIEW MODAL ── */}
      {showPeerReview&&branchColleagues.filter(c=>!peerReviewedIds.includes(c.id)).length>0&&(()=>{
        const pending=branchColleagues.filter(c=>!peerReviewedIds.includes(c.id));
        const current=pending[0];
        const done=branchColleagues.length-pending.length;
        const total=branchColleagues.length;
        return(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.9)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20,overflowY:'auto'}} onClick={()=>setShowPeerReview(false)}>
          <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:28,width:'100%',maxWidth:520,margin:'auto'}} onClick={e=>e.stopPropagation()}>
            {/* Header */}
              ...
              {[['billing_score','Billing & Target Achievement'],['client_score','Client Service'],['teamwork_score','Teamwork & Attitude'],['knowledge_score','Legal Knowledge'],['overall_score','Overall Rating']].map(([key,label])=>(
                <div key={key}>
                  ...
                  <div style={{display:'flex',gap:5}}>
                    {[1,2,3,4,5].map(n=>(
                      <button key={n} style={{flex:1,padding:'7px 4px',borderRadius:6,border:`1px solid ${prForm[key]===n?'#4A90D9':'#252525'}`,background:prForm[key]===n?'rgba(74,144,217,0.12)':'#1A1A1A',color:prForm[key]===n?'#4A90D9':'#555',cursor:'pointer',fontSize:13,fontFamily:'inherit',fontWeight:prForm[key]===n?700:400}} onClick={()=>setPrForm(f=>({...f,[key]:n}))}>
                        {n}★
                      </button>
                    ))}
                  </div>
            ...
            <div style={{display:'flex',gap:8,marginTop:18,justifyContent:'space-between',alignItems:'center'}}>
              <button style={{background:'transparent',border:'1px solid #252525',color:'#555',padding:'6px 14px',borderRadius:6,cursor:'pointer',fontSize:12,fontFamily:'inherit'}} onClick={()=>setShowPeerReview(false)}>Save for Later</button>
              <button style={{background:'#4A90D9',border:'none',color:'#fff',padding:'7px 20px',borderRadius:6,cursor:'pointer',fontSize:12,fontFamily:'inherit',fontWeight:700,opacity:submittingPR||!prForm.overall_score?0.5:1}} disabled={submittingPR||!prForm.overall_score} onClick={async()=>{
                ...
                await supabase.from('feedback_360').insert([{
                  subject_id:current.id,
                  reviewer_id:prForm.is_anonymous?null:userId,
                  reviewer_type:'peer',
                  period:prPeriod,
                  billing_score:prForm.billing_score,
                  client_score:prForm.client_score,
                  teamwork_score:prForm.teamwork_score,
                  knowledge_score:prForm.knowledge_score,
                  overall_score:prForm.overall_score,
                  comments:prForm.comments,
                  is_anonymous:prForm.is_anonymous,
                }]);
                ...
                const remaining=branchColleagues.filter(c=>!newReviewed.includes(c.id));
                if(!remaining.length){
                  setShowPeerReview(false);
                  toast('✓ All peer reviews submitted — thank you!','success');
                }
          ...
          return(<>
            ...
            <div style={C.card}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Matter','Direction','Description','Given To','Due Date','Status','Action'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
              {undertakings.map(u=>{const isOD=u.due_date&&new Date(u.due_date)<new Date()&&u.status==='pending';return(<tr key={u.id}>
                ...
                <td style={C.td}><div style={{display:'flex',gap:4}}>{u.status==='pending'&&<button style={{...C.btn('p'),fontSize:10,padding:'3px 8px'}} onClick={async()=>{await fulfillUndertaking(u.id);fetchUndertakings({userId}).then(r=>setUndertakings(r.undertakings||[])); }}>✓</button>}<button style={{...C.btn('r'),fontSize:10,padding:'3px 8px'}} onClick={async()=>{if(!confirm('Delete?'))return;await deleteUndertaking(u.id);fetchUndertakings({userId}).then(r=>setUndertakings(r.undertakings||[]));}}>✕</button></div></td>
              </tr>);})}
            </tbody></table></div>}
          </>);
        })()}
      ...
      {tab==='communications'&&(<div style={C.main}>
        ...
        <div style={C.card}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Date','Type','Direction','Matter','Subject / Notes','Action'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
          {communications.map(c=>{const typeCol={call:'#4A90D9',email:'#6CC04A',meeting:'#A78BFA',letter:'#EAB308',note:'#888',whatsapp:'#25D366'};return(<tr key={c.id}>
            ...
            <td style={C.td}><button style={{...C.btn('r'),fontSize:10,padding:'3px 8px'}} onClick={async()=>{if(!confirm('Delete?'))return;await deleteClientCommunication(c.id);fetchClientCommunications({userId}).then(r=>setCommunications(r.communications||[])); }}>✕</button></td>
          </tr>);})}
        </tbody></table></div>}
  ...
  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10}}>
    <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Analytics</div><div style={{fontSize:11,color:'#444'}}>{analyticsPeriod==='day'?fdate(selDate):analyticsPeriod==='week'?'This week':fmonth(selDate)} · {getAnalyticsActs(analyticsPeriod).length} sessions</div></div>
  ...
  {(()=>{
    ...
    if(!acts.length) return(<div style={{...C.card,textAlign:'center',padding:'40px',color:'#333'}}><div style={{fontSize:28,marginBottom:10}}>📊</div><div>No data for this period</div></div>);
    return(<>
...
\nBillable Time: ${toHm2(sec)}\nUtilisation: ${util}%\nRevenue Invoiced: R${invAmt.toFixed(2)} incl. VAT\n\nMonthly Breakdown:\n${months.map(m=>{const u=allActs.filter(a=>a.is_billable&&a.date?.startsWith(m)).reduce((s,a)=>s+(a.billing_units||0),0);return new Date(m+'-01T12:00:00').toLocaleString('en-ZA',{month:'long'})+': '+u+' units';}).join('\n')}`;
    ...
    {showCNForm&&cnInvoice&&(<div style={C.modal} onClick={()=>setShowCNForm(false)}><div style={{...C.mbox,maxWidth:420}} onClick={e=>e.stopPropagation()}><div style={{fontSize:15,fontWeight:700,marginBottom:4}}>Issue Credit Note</div><div style={{fontSize:11,color:'#555',marginBottom:16}}>Against invoice <strong style={{color:'#F0F0F0'}}>{cnInvoice.id}</strong> · {cnInvoice.client} · R{((cnInvoice.total_units||0)*(cnInvoice.rate||150)*1.15).toFixed(2)} incl. VAT</div><div style={{display:'flex',flexDirection:'column',gap:12}}><div><label style={C.lbl}>Credit amount (R) *</label><input style={C.inp} type="number" placeholder="0.00" value={cnForm.amount} onChange={e=>setCnForm(f=>({...f,amount:e.target.value}))}/></div><div><label style={C.lbl}>Reason *</label><textarea style={{...C.inp,minHeight:70,resize:'vertical'}} placeholder="Reason for credit note..." value={cnForm.reason} onChange={e=>setCnForm(f=>({...f,reason:e.target.value}))}/></div></div><div style={{display:'flex',gap:8,marginTop:16,justifyContent:'flex-end'}}><button style={C.btn()} onClick={()=>setShowCNForm(false)}>Cancel</button><button style={C.btn('p')} disabled={savingCN||!cnForm.amount||!cnForm.reason.trim()} onClick={handleSaveCreditNote}>{savingCN?'Saving…':'Issue Credit Note'}</button></div></div></div>)}
