...
export default function Manager() {
  ...
  async function sendSatisfactionRequest(matter){ const{data:{session}}=await supabase.auth.getSession(); const res=await fetch('/api/send-satisfaction-request',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${session?.access_token}`},body:JSON.stringify({matterId:matter.id,clientId:matter.client_id||null,attorneyId:matter.user_id,appUrl})}); const data=await res.json(); if(!res.ok){showAlert('Could not send: '+(data.error||'No client email'),'error');return;} showAlert(`✓ Satisfaction survey sent to client.`); }
