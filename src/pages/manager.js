...
export default function Manager() {
  ...
  async function handleSaveCreditNote(){ if(!cnForm.amount||!cnForm.reason.trim()||!cnInvoice) return; setSavingCN(true); const{error}=await saveCreditNote({invoiceId:cnInvoice.id,client:cnInvoice.client,matterId:cnInvoice.matter_id,amount:cnForm.amount,reason:cnForm.reason},profile?.id); setSavingCN(false); if(error){showAlert('Error: '+error.message,'error');return;} const{creditNotes:cn}=await fetchCreditNotes(null); setCreditNotes(cn); setShowCNForm(false);setCnInvoice(null);setCnForm({amount:'',reason:''});showAlert('âœ“ Credit note issued.'); await logAudit('credit_note_issued','invoice',cnInvoice.id,{amount:cnForm.amount,reason:cnForm.reason},profile?.id); }
  async function handleEmailInvoice(inv,customEmail){ setEmailingInv(inv.id); const res=await fetch('/api/send-invoice',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({invoiceId:inv.id,recipientEmail:customEmail||''})}); const data=await res.json(); setEmailingInv(null); if(!res.ok){showAlert('Could not send: '+(data.error||'Unknown error'),'error');return;} showAlert(data.warning||`âœ“ Invoice emailed to ${data.sentTo}`); await logAudit('invoice_emailed','invoice',inv.id,{sentTo:customEmail},profile?.id); }
  ...
  async function handleSaveUndertaking(){ if(!utForm.description.trim()||!utForm.matter_id) return; const{error}=await saveUndertaking({...utForm},profile?.id); if(error){showAlert('Error: '+error.message,'error');return;} showAlert('âœ“ Undertaking saved.'); setShowUTForm(false); setUtForm({matter_id:'',direction:'given',description:'',given_to:'',due_date:'',notes:''}); const{undertakings:ut}=await fetchUndertakings({}); setUndertakings(ut); await logAudit('undertaking_created','matter',utForm.matter_id,{description:utForm.description},profile?.id); }
  async function handleSaveComm(){ if(!commForm.body.trim()) return; const{error}=await saveClientCommunication({...commForm},profile?.id); if(error){showAlert('Error: '+error.message,'error');return;} showAlert('âœ“ Communication logged.'); setShowCommForm(false); setCommForm({client_id:'',matter_id:'',comm_type:'call',direction:'outbound',subject:'',body:'',comm_date:new Date().toLocaleDateString('en-CA')}); const{communications:co}=await fetchClientCommunications({}); setCommunications(co); await logAudit('communication_logged','client',commForm.client_id,{type:commForm.comm_type},profile?.id); }
  async function handleCloseMatter(m){ if(!closureForm.closure_notes.trim()){showAlert('Please add closure notes.','error');return;} const{error}=await updateMatter(m.id,{status:'closed',closure_notes:closureForm.closure_notes,closed_at:new Date().toISOString(),closed_by:profile?.id}); if(error){showAlert('Error: '+error.message,'error');return;} showAlert(`âœ“ Matter ${m.id} closed.`); setClosingMatter(null); setClosureForm({closure_notes:''}); load(); await logAudit('matter_closed','matter',m.id,{notes:closureForm.closure_notes},profile?.id); }
  async function handleAddInterest(inv){ const age=Math.floor((new Date()-new Date(inv.created_at||0))/86400000); const rate=10.5; const outstanding=Math.max(0,(inv.total_units||0)*(inv.rate||150)*1.15-invoicePayments.filter(p=>p.invoice_id===inv.id).reduce((s,p)=>s+Number(p.amount),0)); const interest=parseFloat((outstanding*(rate/100)*(age/365)).toFixed(2)); if(interest<=0){showAlert('No interest due â€” invoice is not overdue.','error');return;} if(!confirm(`Add R${interest} interest charge (${rate}% p.a. Ã— ${age} days) to invoice ${inv.id}?`)) return; const{error}=await saveInterestCharge({invoiceId:inv.id,amount:interest,ratePercent:rate,daysOverdue:age},profile?.id); if(error){showAlert('Error: '+error.message,'error');return;} showAlert(`âœ“ Interest charge of R${interest} added.`); await logAudit('interest_charged','invoice',inv.id,{amount:interest,days:age},profile?.id); }
  async function handleApproveinvoice(invId){ const{error}=await supabase.from('invoices').update({status:'approved',approved_by:profile?.id,approved_at:new Date().toISOString()}).eq('id',invId); if(error){showAlert('Error: '+error.message,'error');return;} showAlert('âœ“ Invoice approved.'); load(); await logAudit('invoice_approved','invoice',invId,{},profile?.id); }
  async function handleSavePayPlan(){ if(!payPlanForm.instalment||!payPlanInv) return; setSavingPlan(true); const total=(payPlanInv.total_units||0)*(payPlanInv.rate||150)*1.15; const months=Math.ceil(total/parseFloat(payPlanForm.instalment)); const instalments=Array.from({length:months},(_,i)=>{ const d=new Date(payPlanForm.start_date+'T12:00:00'); d.setMonth(d.getMonth()+i); const isLast=i===months-1; return{due_date:d.toLocaleDateString('en-CA'),amount:isLast?(total-(months-1)*parseFloat(payPlanForm.instalment)).toFixed(2):parseFloat(payPlanForm.instalment),status:'pending'}; }); const{error}=await supabase.from('payment_plans').insert([{invoice_id:payPlanInv.id,client_id:payPlanInv.client_id||null,total_amount:total,instalment:parseFloat(payPlanForm.instalment),frequency:payPlanForm.frequency,start_date:payPlanForm.start_date,notes:payPlanForm.notes,created_by:profile?.id}]).select(); setSavingPlan(false); if(error){showAlert('Error: '+error.message,'error');return;} showAlert(`âœ“ Payment plan created â€” ${months} instalments of R${payPlanForm.instalment}`); setShowPayPlan(false); setPayPlanInv(null); }
  async function handleSaveOpp(){ if(!oppMatter) return; const{error}=await supabase.from('matters').update({opposing_party:oppForm.opposing_party,opposing_attorney:oppForm.opposing_attorney,opposing_firm:oppForm.opposing_firm}).eq('id',oppMatter.id); if(error){showAlert('Error: '+error.message,'error');return;} showAlert('âœ“ Opposing party saved.'); setShowOppForm(false); load(); }
  async function sendSatisfactionRequest(matter){ const res=await fetch('/api/send-satisfaction-request',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({matterId:matter.id,clientId:matter.client_id||null,attorneyId:matter.user_id,appUrl})}); const data=await res.json(); if(!res.ok){showAlert('Could not send: '+(data.error||'No client email'),'error');return;} showAlert(`âœ“ Satisfaction survey sent to client.`); }
  async function updateFicaRisk(clientId,rating){ await supabase.from('clients').update({risk_rating:rating}).eq('id',clientId); showAlert('âœ“ Risk rating updated.'); load(); }

  async function sendOverdueReminders(){ const now=new Date(); const overdue=invoices.filter(inv=>{ const age=Math.floor((now-new Date(inv.created_at||0))/86400000); const paid=invoicePayments.filter(p=>p.invoice_id===inv.id).reduce((s,p)=>s+Number(p.amount),0); return age>30&&Math.max(0,(inv.total_units||0)*(inv.rate||150)*1.15-paid)>0&&!inv.written_off; }); if(!overdue.length){showAlert('No overdue invoices to remind.','error');return;} let sent=0; for(const inv of overdue){ const res=await fetch('/api/send-invoice',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({invoiceId:inv.id,type:'reminder'})}); if(res.ok) sent++; } showAlert(`âœ“ Sent ${sent} overdue reminders.`); }
  async function approvePayment(id){ const {error}=await supabase.from('trust_transactions').update({status:'posted',approved_by:profile?.id,approved_at:new Date().toISOString()}).eq('id',id); if(error){showAlert('Error: '+error.message,'error');return;} showAlert('âœ“ Payment approved and posted.','success'); load(); }
  ...
  async function assignBranch(userId,branchId){ const {error}=await supabase.from('profiles').update({branch_id:branchId}).eq('id',userId); if(error){showAlert('Error: '+error.message,'error');return;} showAlert('âœ“ Branch updated.','success'); load(); }
  async function saveTitle(userId,title){ await supabase.from('profiles').update({title}).eq('id',userId); setProfiles(prev=>prev.map(p=>p.id===userId?{...p,title}:p)); setEditingTitle(prev=>({...prev,[userId]:false})); showAlert('âœ“ Title updated.'); }
  async function removeStaff(userId,name){ if(!confirm(`Remove ${name} from the system? This cannot be undone.`)) return; const res=await fetch('/api/remove-staff',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId})}); const data=await res.json(); if(!res.ok){showAlert('Error: '+data.error,'error');return;} showAlert(`âœ“ ${name} removed.`,'success'); load(); }
  ...
  async function handleInvite(){
    ...
    showAlert(`âœ“ ${inviteForm.fullName} added to ${branchName}. Login details emailed to ${inviteForm.email}. Temporary password: ${tempPassword} â€” shown here as backup.`,'success');
    setInviting(false);
    setShowInvite(false);
  ...
  const byAtty=filteredProfiles.map(p=>{
    ...
    return{...p,branch_name:br?.name||'â€”',total_sec:periodP.reduce((s,a)=>s+(a.duration_seconds||0),0),bill_sec:periodBill.reduce((s,a)=>s+(a.duration_seconds||0),0),all_units:allUnits,billed_units:billedU,unbilled_units:unbilledU,invoiceCount:attyInvs.length};
  ...
  const byAttyAllTime=filteredProfiles.map(p=>{
    ...
    const br=branches.find(b=>b.id===p.branch_id);
    return{...p,branch_name:br?.name||'â€”',bill_sec:allTimeP.filter(a=>a.is_billable).reduce((s,a)=>s+(a.duration_seconds||0),0),all_units:allUnits,billed_units:billedU,unbilled_units:Math.max(0,allUnits-billedU),invoiceCount:attyInvs.length};
  }).sort((a,b)=>b.all_units-a.all_units);
  ...
  return(
    <>
      <Head><title>MB SmartTrack â€” Manager</title></Head>
...
{trustAlert.msg&&(<div style={{background:trustAlert.type==='error'?'rgba(220,80,80,0.1)':'rgba(141,198,63,0.1)',border:`1px solid ${trustAlert.type==='error'?'rgba(220,80,80,0.4)':'rgba(141,198,63,0.3)'}`,padding:'14px 24px',fontSize:12,color:trustAlert.type==='error'?'#E05252':'#8DC63F',display:'flex',justifyContent:'space-between',alignItems:'center',gap:16}}>
  ...
  {trustAlert.type==='success'&&trustAlert.msg.includes('Temporary password:')&&(
    <button style={{background:'rgba(141,198,63,0.2)',border:'1px solid rgba(141,198,63,0.4)',color:'#8DC63F',padding:'4px 12px',borderRadius:6,cursor:'pointer',fontSize:11,fontFamily:'inherit',whiteSpace:'nowrap'}} onClick={()=>{ const pwd=trustAlert.msg.match(/Temporary password: ([^\sâ€”]+)/)?.[1]; if(pwd){navigator.clipboard.writeText(pwd);} }}>ðŸ“‹ Copy Password</button>
  )}
  <button style={{background:'none',border:'none',color:'inherit',cursor:'pointer',flexShrink:0}} onClick={()=>setTrustAlert({msg:'',type:''})}>âœ•</button>
</div>)}
        {pendingPayments.length>0&&tab!=='trust'&&(<div style={{background:'rgba(234,179,8,0.1)',border:'1px solid rgba(234,179,8,0.3)',padding:'10px 24px',fontSize:12,color:'#EAB308',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span>â³ {pendingPayments.length} trust payment{pendingPayments.length>1?'s':''} pending your approval â€” {fmtR(pendingPayments.reduce((s,t)=>s+Number(t.amount),0))}</span><button style={C.btn('warn')} onClick={()=>setTab('trust')}>Review approvals â†’</button></div>)}
        {(()=>{ const ficaMap=Object.fromEntries(ficaRecords.map(r=>[r.client_id,r])); const pending=clients.filter(c=>{ const r=ficaMap[c.id]; return !r||r.fica_status==='pending'||r.fica_status==='expired'; }).length; return pending>0&&tab!=='clients'?(<div style={{background:'rgba(220,80,80,0.08)',border:'1px solid rgba(220,80,80,0.25)',padding:'10px 24px',fontSize:12,color:'#E05252',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span>ðŸªª {pending} client{pending!==1?'s':''} with FICA pending or expired</span><button style={{...C.btn('r'),fontSize:11}} onClick={()=>setTab('clients')}>Review â†’</button></div>):null; })()}

        {tab==='overview'&&(<div style={C.main}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
            <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Firm Overview â€” Motsoeneng Bill</div><div style={{fontSize:11,color:'#444'}}>{overviewPeriod==='day'?fdate(selDate):overviewPeriod==='week'?'This week':overviewPeriod==='month'?new Date(selDate.substring(0,7)+'-01T12:00:00').toLocaleDateString('en-ZA',{month:'long',year:'numeric'}):'All time'} Â· {profiles.length} staff Â· {branches.length} branches</div></div>
          ...
          <div style={C.card}>
            <div style={{fontSize:12,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>Attorney Leaderboard â€” {overviewPeriod==='day'?fdate(selDate):overviewPeriod==='week'?'This Week':overviewPeriod==='month'?new Date(selDate.substring(0,7)+'-01T12:00:00').toLocaleDateString('en-ZA',{month:'long',year:'numeric'}):'All Time'}</div>
            <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['#','Attorney','Branch','Billable Time','Units Earned','Target','Performance','Units Billed','Unbilled','Invoices'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>{!byAtty.length&&<tr><td colSpan={10} style={{padding:'30px',textAlign:'center',color:'#333',fontSize:13}}>No data yet.</td></tr>}{byAtty.map((a,i)=>{const target=a.monthly_target||0;const pct=target>0?Math.round((a.all_units/target)*100):null;const perfColor=pct===null?'#555':pct>=100?'#8DC63F':pct>=70?'#EAB308':'#E05252';return(<tr key={a.id}><td style={{...C.td,color:'#444',fontWeight:600,width:28}}>{i+1}</td><td style={{...C.td,fontWeight:500,color:'#D0D0D0'}}>{a.full_name}<div style={{fontSize:9,color:'#444'}}>{a.email}</div></td><td style={{...C.td,fontSize:10}}><span style={{background:'rgba(74,144,217,0.1)',color:'#4A90D9',padding:'2px 8px',borderRadius:20,fontSize:9,fontWeight:600}}>{a.branch_name}</span></td><td style={{...C.td,fontFamily:'monospace',color:'#8DC63F'}}>{toHm(a.bill_sec)||'0m'}</td><td style={{...C.td,fontFamily:'monospace',color:'#8DC63F',fontWeight:700}}>{a.all_units||'â€”'}</td><td style={{...C.td,fontFamily:'monospace',color:'#555'}}>{target>0?target:'â€”'}</td><td style={C.td}>{pct!==null?(<div style={{display:'flex',alignItems:'center',gap:6}}><div style={{flex:1,height:6,background:'#1A1A1A',borderRadius:3}}><div style={{width:`${Math.min(pct,100)}%`,height:'100%',background:perfColor,borderRadius:3}}/></div><span style={{fontSize:10,color:perfColor,fontWeight:700,minWidth:35}}>{pct}%</span></div>):<span style={{color:'#333',fontSize:10}}>No target</span>}</td><td style={{...C.td,fontFamily:'monospace',color:'#8DC63F'}}>{a.billed_units||'â€”'}</td><td style={{...C.td,fontFamily:'monospace',color:a.unbilled_units>0?'#EAB308':'#444'}}>{a.unbilled_units>0?a.unbilled_units:'â€”'}</td><td style={{...C.td,fontFamily:'monospace',color:'#777'}}>{a.invoiceCount}</td></tr>);})}</tbody></table></div>
          </div>
          ...
          <div style={{marginTop:14,textAlign:'center',fontSize:11,color:'#252525'}}>Motsoeneng Bill Â· MB SmartTrack Manager View</div>
        </div>)}

        {tab==='trust'&&(<div style={C.main}>
          <div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em',marginBottom:4}}>Trust Accounting</div>
          <div style={{fontSize:11,color:'#444',marginBottom:16}}>All branches Â· Legal Practice Act compliant</div>
  ...
  <div style={C.card}><div style={{fontSize:12,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>Attorney Performance</div><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Attorney','Branch','Billable Time','Units Earned','Units Billed','Unbilled','Est. Value'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>{!byAtty.length&&<tr><td colSpan={7} style={{...C.td,textAlign:'center',color:'#333',padding:30}}>No billable data for this period.</td></tr>}{byAtty.map(a=>(<tr key={a.id}><td style={{...C.td,fontWeight:500,color:'#D0D0D0'}}>{a.full_name}<div style={{fontSize:9,color:'#444'}}>{a.email}</div></td><td style={{...C.td,fontSize:10}}><span style={{background:'rgba(74,144,217,0.1)',color:'#4A90D9',padding:'2px 8px',borderRadius:20,fontSize:9}}>{a.branch_name}</span></td><td style={{...C.td,fontFamily:'monospace',color:'#8DC63F'}}>{toHm(a.bill_sec)||'0m'}</td><td style={{...C.td,fontFamily:'monospace',color:'#8DC63F',fontWeight:700}}>{a.all_units||'â€”'}</td><td style={{...C.td,fontFamily:'monospace',color:'#8DC63F'}}>{a.billed_units||'â€”'}</td><td style={{...C.td,fontFamily:'monospace',color:a.unbilled_units>0?'#EAB308':'#444'}}>{a.unbilled_units>0?a.unbilled_units:'â€”'}</td><td style={{...C.td,fontFamily:'monospace',color:a.unbilled_units>0?'#EAB308':'#444',fontWeight:600}}>{a.unbilled_units>0?`R${(a.unbilled_units*rate).toLocaleString()}`:'â€”'}</td></tr>))}</tbody></table></div>
</div>)}

        {tab==='history'&&(<div style={C.main}>
  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
    <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Firm History</div><div style={{fontSize:11,color:'#444'}}>{histLoading?'Loadingâ€¦':'Click a month to see attorney billing details'}</div></div>
  ...
  {!histLoading&&histData.every(m=>m.sessions===0)&&(<div style={{...C.card,textAlign:'center',padding:'30px 20px',marginBottom:14}}><div style={{fontSize:22,marginBottom:8}}>ðŸ“Š</div><div style={{fontSize:13,color:'#555',marginBottom:4}}>No activity data for {histYear}</div><div style={{fontSize:11,color:'#333'}}>Activities tracked by the Electron agent will appear here once attorneys start recording time.</div></div>)}
  {monthBars.length>0&&(<div style={{...C.card,marginBottom:14}}><div style={{fontSize:11,fontWeight:600,color:'#D0D0D0',marginBottom:4}}>Billing units by month â€” {histYear}</div><BarChart data={monthBars} height={130}/></div>)}
  <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:14}}>
    {histData.map(m=>{
  ...
  {selMonth&&(<div style={C.card}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
      <div style={{fontSize:13,fontWeight:600,color:'#D0D0D0'}}>{fmonth(selMonth)} â€” Attorney Billing</div>
      <button style={{...C.btn(),fontSize:11}} onClick={()=>{setSelMonth(null);setMonthActs([]);}}>âœ• Close</button>
    </div>
  ...
  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
    <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Work In Progress â€” WIP Report</div><div style={{fontSize:11,color:'#444'}}>Billable work not yet invoiced Â· {new Date().toLocaleDateString('en-ZA',{weekday:'long',day:'2-digit',month:'long',year:'numeric'})}</div></div>
  ...
  {(()=>{
    const wipData=filteredProfiles.map(p=>{
      ...
      if(unassignedUnits>0) wipMatters.push({matterId:'â€”',units:unassignedUnits,billedUnits:0,unbilled:unassignedUnits,matter:{client:'âš  No matter assigned â€” needs attention'}});
  ...
  {(()=>{
    ...
    const buckets={'0-30':{label:'Current (0â€“30 days)',color:'#8DC63F',invs:[]},'31-60':{label:'30â€“60 days',color:'#4A90D9',invs:[]},'61-90':{label:'60â€“90 days',color:'#EAB308',invs:[]},'91-120':{label:'90â€“120 days',color:'#E07B30',invs:[]},'120+':{label:'120+ days',color:'#E05252',invs:[]}};
    ...
    return(<>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
        <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Debtors Age Analysis</div><div style={{fontSize:11,color:'#444'}}>Outstanding invoices Â· {new Date().toLocaleDateString('en-ZA',{day:'2-digit',month:'long',year:'numeric'})}</div></div>
      ...
      <div style={{...C.card,marginBottom:14}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><div style={{fontSize:12,fontWeight:600,color:'#D0D0D0'}}>Outstanding by Client</div><div style={{fontSize:13,fontWeight:700,color:'#EAB308'}}>{fmtR(totalOut)} total outstanding</div></div>
        {!clients.length?<div style={{textAlign:'center',padding:30,color:'#555',fontSize:12}}>âœ… All invoices paid</div>:
        <table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Client','Invoices','0â€“30','31â€“60','61â€“90','90â€“120','120+','Total Outstanding'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
          {clients.map(c=>{
            ...
            return(<tr key={c.client}>
              ...
              {Object.entries(bkts).map(([k,v])=><td key={k} style={{...C.td,fontFamily:'monospace',color:v>0?buckets[k].color:'#333',fontWeight:v>0?700:400}}>{v>0?fmtR(v):'â€”'}</td>)}
      ...
      <div style={C.card}><div style={{fontSize:12,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>Invoice Detail</div>
        <table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Invoice','Client','Matter','Date','Age','Invoice Amt','Paid','Outstanding','Actions'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
          ...
          {unpaidInvs.sort((a,b)=>age(b)-age(a)).map(inv=>{const a=age(inv),p=paid(inv.id),o=outstanding(inv),amt=(inv.total_units||0)*(inv.rate||150)*1.15;return(<tr key={inv.id} style={{opacity:inv.written_off?0.5:1}}>
            ...
            <td style={{...C.td,fontFamily:'monospace',color:'#A78BFA',fontSize:10}}>{inv.matter_id||'â€”'}</td>
            ...
            <td style={{...C.td,fontFamily:'monospace',color:p>0?'#8DC63F':'#333'}}>{p>0?fmtR(p):'â€”'}</td>
            ...
            <td style={{...C.td}}><div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
              <button style={{...C.btn(),fontSize:10,padding:'3px 8px'}} disabled={emailingInv===inv.id} onClick={()=>{const em=prompt('Send to email:',inv.client_email||'');if(em===null)return;handleEmailInvoice(inv,em);}}>{emailingInv===inv.id?'â€¦':'âœ‰'}</button>
  ...
  {showPayForm&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowPayForm(false)}>
    <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:28,width:'100%',maxWidth:440}} onClick={e=>e.stopPropagation()}>
      ...
      {(()=>{const lbl={fontSize:10,color:'#555',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:4,display:'block'};const inp={background:'#1A1A1A',border:'1px solid #252525',color:'#F0F0F0',padding:'9px 12px',borderRadius:6,fontSize:12,fontFamily:'inherit',width:'100%',boxSizing:'border-box'};return(<div style={{display:'flex',flexDirection:'column',gap:12}}>
        <div><label style={lbl}>Invoice *</label><select style={inp} value={payForm.invoiceId} onChange={e=>setPayForm(f=>({...f,invoiceId:e.target.value}))}><option value="">â€” Select invoice â€”</option>{invoices.filter(inv=>{const p=invoicePayments.filter(x=>x.invoice_id===inv.id).reduce((s,x)=>s+Number(x.amount),0);return Math.max(0,(inv.total_units||0)*(inv.rate||150)*1.15-p)>0;}).map(inv=><option key={inv.id} value={inv.id}>{inv.id} Â· {inv.client} Â· {fmtR((inv.total_units||0)*(inv.rate||150)*1.15)}</option>)}</select></div>
  ...
  {(()=>{
    ...
    return(<>
      <div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em',marginBottom:4}}>Financial Reports â€” Motsoeneng Bill</div>
      ...
      <div style={C.card}><div style={{fontSize:12,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>Monthly Revenue â€” All Time</div>
        <table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Month','Invoices','Units','Invoiced (incl. VAT)','Collected','Outstanding','Collection %'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
          ...
          {months.map(m=>{const pct=m.invoiced>0?Math.round(m.collected/m.invoiced*100):0;return(<tr key={m.month}>
            ...
            <td style={{...C.td,fontFamily:'monospace',color:'#4A90D9'}}>{m.collected>0?fmtR(m.collected):'â€”'}</td>
            <td style={{...C.td,fontFamily:'monospace',color:m.invoiced-m.collected>0?'#EAB308':'#555'}}>{m.invoiced-m.collected>0?fmtR(m.invoiced-m.collected):'â€”'}</td>
      ...
      <div style={C.card}><div style={{fontSize:12,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>Attorney Revenue Summary</div>
        <table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Attorney','Branch','Units Billed','Total Invoiced','Collected','Outstanding'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
          {attyRev.map(a=><tr key={a.id}><td style={{...C.td,fontWeight:500,color:'#D0D0D0'}}>{a.full_name}</td><td style={{...C.td,fontSize:10}}><span style={{background:'rgba(74,144,217,0.1)',color:'#4A90D9',padding:'2px 8px',borderRadius:20,fontSize:9}}>{branches.find(b=>b.id===a.branch_id)?.name||'â€”'}</span></td><td style={{...C.td,fontFamily:'monospace',color:'#8DC63F'}}>{a.units||'â€”'}</td><td style={{...C.td,fontFamily:'monospace',color:'#8DC63F',fontWeight:700}}>{a.invoiced>0?fmtR(a.invoiced):'â€”'}</td><td style={{...C.td,fontFamily:'monospace',color:'#4A90D9'}}>{a.collected>0?fmtR(a.collected):'â€”'}</td><td style={{...C.td,fontFamily:'monospace',color:a.outstanding>0?'#EAB308':'#555'}}>{a.outstanding>0?fmtR(a.outstanding):'â€”'}</td></tr>)}
        </tbody></table>
      </div>
    </>);
  })()}
  ...
  {(()=>{
    ...
    function printStatement(c){
      const rows=c.invoices.map(inv=>{const p=paid(inv.id),amt=(inv.total_units||0)*(inv.rate||150),vat=amt*.15,total=amt*1.15,out=Math.max(0,total-p);return`<tr><td>${fmtDate(inv.created_at?.substring(0,10))}</td><td style="font-family:monospace">${inv.id}</td><td>${inv.matter_name||inv.matter_id||'â€”'}</td><td align="right">${inv.total_units||0}</td><td align="right">R${amt.toLocaleString()}</td><td align="right">R${vat.toFixed(2)}</td><td align="right">R${total.toFixed(2)}</td><td align="right" style="color:${p>0?'#16a34a':'#666'}">${p>0?'R'+p.toFixed(2):'â€”'}</td><td align="right" style="font-weight:700;color:${out>0?'#dc2626':'#16a34a'}">${out>0?'R'+out.toFixed(2):'PAID'}</td></tr>`;}).join('');
      const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Statement â€” ${c.client}</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;color:#111;padding:40px;max-width:900px;margin:auto}.top{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #8DC63F;padding-bottom:16px;margin-bottom:20px}.logo{font-size:26px;font-weight:900;letter-spacing:-0.04em}.logo span{color:#8DC63F}table{width:100%;border-collapse:collapse;margin:12px 0}th{background:#f8f8f8;padding:8px;font-size:9px;text-transform:uppercase;color:#999;border-bottom:2px solid #eee;text-align:left}td{padding:7px 8px;font-size:11px;border-bottom:1px solid #f3f3f3}.summary{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;background:#f9f9f9;border-radius:8px;padding:16px;margin-bottom:16px}.lbl{font-size:9px;text-transform:uppercase;color:#aaa;margin-bottom:3px}.val{font-size:16px;font-weight:700}.foot{margin-top:20px;padding-top:12px;border-top:1px solid #eee;font-size:10px;color:#ccc;text-align:center}@media print{body{padding:20px}}</style></head><body><div class="top"><div><div class="logo">M<span>B</span></div><div style="font-size:11px;color:#999;margin-top:2px">Motsoeneng Bill</div></div><div style="text-align:right"><h2>ACCOUNT STATEMENT</h2><div style="font-size:11px;color:#999">Client: ${c.client} Â· ${new Date().toLocaleDateString('en-ZA')}</div></div></div><div class="summary"><div><div class="lbl">Total Invoiced</div><div class="val">R${c.billed.toFixed(2)}</div></div><div><div class="lbl">Total Paid</div><div class="val" style="color:#16a34a">R${c.paid.toFixed(2)}</div></div><div><div class="lbl">Balance Due</div><div class="val" style="color:${c.billed-c.paid>0?'#dc2626':'#16a34a'}">${c.billed-c.paid>0?'R'+(c.billed-c.paid).toFixed(2):'PAID IN FULL'}</div></div></div><table><thead><tr><th>Date</th><th>Invoice</th><th>Matter</th><th align="right">Units</th><th align="right">Excl. VAT</th><th align="right">VAT 15%</th><th align="right">Total</th><th align="right">Paid</th><th align="right">Balance</th></tr></thead><tbody>${rows||'<tr><td colspan="9" style="text-align:center;color:#ccc;padding:16px">No invoices</td></tr>'}</tbody></table><div class="foot">Motsoeneng Bill Â· VAT Reg: 4100000000 Â· accounts@mb.co.za<br><em>This statement was generated on ${new Date().toLocaleDateString('en-ZA')} and is subject to errors and omissions.</em></div><script>window.onload=function(){window.print()}<\/script></body></html>`;
    ...
    return(<>
      ...
      <div style={{fontSize:11,color:'#444',marginBottom:14}}>Per-client account statements Â· Print or email to clients</div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:14}}>
      ...
      {!clients.length?(<div style={{...C.card,textAlign:'center',padding:40,color:'#555'}}><div style={{fontSize:28,marginBottom:10}}>ðŸ“‹</div><div>No invoices yet</div></div>):clients.map(c=>{const out=Math.max(0,c.billed-c.paid);return(<div key={c.client} style={C.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}><div><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0'}}>{c.client}</div><div style={{fontSize:10,color:'#555'}}>{c.invoices.length} invoice{c.invoices.length!==1?'s':''}</div></div><div style={{display:'flex',gap:16,alignItems:'center'}}><div style={{textAlign:'right'}}><div style={{fontSize:9,color:'#555',textTransform:'uppercase',marginBottom:2}}>Invoiced</div><div style={{fontSize:15,fontWeight:700,color:'#8DC63F'}}>{fmtR(c.billed)}</div></div><div style={{textAlign:'right'}}><div style={{fontSize:9,color:'#555',textTransform:'uppercase',marginBottom:2}}>Paid</div><div style={{fontSize:15,fontWeight:700,color:'#4A90D9'}}>{fmtR(c.paid)}</div></div><div style={{textAlign:'right'}}><div style={{fontSize:9,color:'#555',textTransform:'uppercase',marginBottom:2}}>Balance</div><div style={{fontSize:15,fontWeight:700,color:out>0?'#EAB308':'#8DC63F'}}>{out>0?fmtR(out):'Paid âœ“'}</div></div><button style={C.btn('p')} onClick={()=>printStatement(c)}>Print Statement</button></div></div></div>);})}
    </>);
  })()}
</div>)}
  ...
  {(()=>{
    ...
    return(<>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
        <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Clients</div><div style={{fontSize:11,color:'#444'}}>{clients.length} clients Â· Firm-wide</div></div>
        <div style={{display:'flex',gap:8}}><button style={C.btn()} onClick={()=>router.push('/clients')}>Open Full CRM â†’</button></div>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:14}}>
      ...
      <div style={C.card}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Ref','Client','Type','Email','FICA','Risk','Matters'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
        {!clients.length&&<tr><td colSpan={6} style={{...C.td,textAlign:'center',color:'#333',padding:30}}>No clients yet. <button style={{...C.btn('p'),fontSize:11,marginLeft:8}} onClick={()=>router.push('/clients')}>Add first client â†’</button></td></tr>}
        {clients.slice(0,50).map(c=>{const fs=ficaStatus(c.id);const fst=FSTA[fs]||FSTA.pending;const cm=matters.filter(m=>m.client_id===c.id);return(<tr key={c.id} style={{cursor:'pointer'}} onClick={()=>router.push('/clients')}>
          <td style={{...C.td,fontFamily:'monospace',fontSize:10,color:'#A78BFA'}}>{c.client_no||'â€”'}</td>
          ...
          <td style={{...C.td,fontSize:10,color:'#555'}}>{c.email||'â€”'}</td>
  ...
  {(()=>{
    ...
    return(<>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
        <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Disbursements</div><div style={{fontSize:11,color:'#444'}}>Costs advanced â€” all attorneys</div></div>
      ...
      <div style={C.card}><div style={{fontSize:12,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>All Disbursements</div>
        <table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Cat','Date','Matter','Description','Qty','Amount','Status','Actions'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
          ...
          {disbursements.map(d=>(<tr key={d.id}>
            <td style={{...C.td,width:28,textAlign:'center'}}>{cats[d.category]||'ðŸ“Ž'}</td>
            <td style={{...C.td,fontSize:10,color:'#666'}}>{d.date}</td>
            <td style={{...C.td,fontFamily:'monospace',color:'#A78BFA',fontSize:10}}>{d.matter_id||'â€”'}</td>
      ...
      {showDisbForm&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowDisbForm(false)}>
        <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:28,width:'100%',maxWidth:460}} onClick={e=>e.stopPropagation()}>
          ...
          {(()=>{const lbl2={fontSize:10,color:'#555',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:4,display:'block'};const inp2={background:'#1A1A1A',border:'1px solid #252525',color:'#F0F0F0',padding:'9px 12px',borderRadius:6,fontSize:12,fontFamily:'inherit',width:'100%',boxSizing:'border-box'};return(<div style={{display:'flex',flexDirection:'column',gap:12}}>
            <div><label style={lbl2}>Matter *</label><select style={inp2} value={disbForm.matter_id} onChange={e=>setDisbForm(f=>({...f,matter_id:e.target.value}))}><option value="">â€” Select matter â€”</option>{matters.map(m=><option key={m.id} value={m.id}>{m.id} Â· {m.client}</option>)}</select></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            ...
            <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:6}}>
              ...
              <button style={C.btn('p')} onClick={async()=>{if(!disbForm.matter_id||!disbForm.description||!disbForm.amount){showAlert('Fill in all required fields.','error');return;}const{error}=await saveDisbursement({matter_id:disbForm.matter_id,date:disbForm.date,category:disbForm.category,description:disbForm.description,amount:parseFloat(disbForm.amount)*disbForm.quantity,quantity:disbForm.quantity,reference:disbForm.reference,branch_id:profile?.branch_id||null,status:'unbilled'},profile?.id);if(error){showAlert('Error: '+error.message,'error');return;}showAlert('âœ“ Disbursement added.');setShowDisbForm(false);load();}}>Add</button>
            </div>
          </div>);})()}
        </div>
      </div>)}
  ...
  <div style={C.card}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Name','Rate per Unit (R)','Description','Default','Actions'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
    ...
    {feeSchedules.map(s=>(<tr key={s.id}>
      ...
      <td style={{...C.td,color:'#666'}}>{s.description||'â€”'}</td>
      <td style={{...C.td,textAlign:'center'}}>{s.is_default?<span style={{color:'#8DC63F',fontWeight:700}}>âœ“ Default</span>:'â€”'}</td>
    ...
    <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:28,width:'100%',maxWidth:400}} onClick={e=>e.stopPropagation()}>
      ...
      {(()=>{const lbl2={fontSize:10,color:'#555',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:4,display:'block'};const inp2={background:'#1A1A1A',border:'1px solid #252525',color:'#F0F0F0',padding:'9px 12px',borderRadius:6,fontSize:12,fontFamily:'inherit',width:'100%',boxSizing:'border-box'};return(<div style={{display:'flex',flexDirection:'column',gap:12}}>
        ...
        {tab==='invoices'&&(<div style={C.main}>
          <div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em',marginBottom:14}}>All Invoices â€” Motsoeneng Bill</div>
        ...
        {tab==='settings'&&(<div style={C.main}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:10}}>
            ...
            <button style={C.btn()} onClick={()=>router.push('/settings')}>Open full settings â†’</button>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
            <div style={C.card}>
            ...
            <div style={C.card}>
              ...
              <button style={{...C.btn('p'),marginTop:8,width:'100%'}} onClick={()=>router.push('/settings')}>Edit all settings â†’</button>
            </div>
          </div>
        </div>)}
        ...
        {tab==='staff'&&(<div style={C.main}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10}}>
            <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Staff Management</div><div style={{fontSize:11,color:'#444',marginTop:2}}>{profiles.length} staff members Â· {branches.length} branches Â· No IT needed</div></div>
          ...
          <div style={C.card}>
            ...
            <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Name','Title','Role','Branch','Change Branch','Rate (R)','Monthly Target (units)','Remove'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>{!profiles.length&&<tr><td colSpan={8} style={{padding:'30px',textAlign:'center',color:'#333'}}>No staff yet</td></tr>}{profiles.map(p=>{ const br=branches.find(b=>b.id===p.branch_id); return(<tr key={p.id}><td style={{...C.td,fontWeight:500,color:'#D0D0D0'}}>{p.full_name}<div style={{fontSize:10,color:'#555'}}>{p.email||'â€”'}</div></td><td style={C.td}>{editingTitle[p.id]?(<input autoFocus type="text" defaultValue={p.title||''} placeholder="e.g. Senior Attorney" style={{background:'#1A1A1A',border:'1px solid #4A90D9',color:'#F0F0F0',padding:'4px 8px',borderRadius:6,fontSize:11,width:160,fontFamily:'inherit'}} onBlur={e=>saveTitle(p.id,e.target.value)} onKeyDown={e=>{if(e.key==='Enter')e.target.blur();if(e.key==='Escape')setEditingTitle(prev=>({...prev,[p.id]:false}));}}/>) :(<span style={{fontSize:11,color:p.title?'#C8C8C8':'#4A90D9',cursor:'pointer',textDecoration:'underline',textDecorationStyle:'dotted'}} onClick={()=>setEditingTitle(prev=>({...prev,[p.id]:true}))} title="Click to edit">{p.title||'Set title'}</span>)}</td><td style={C.td}><span style={{fontSize:9,padding:'2px 8px',borderRadius:20,fontWeight:600,background:roleBg(p.role),color:roleColor(p.role)}}>{p.role||'attorney'}</span></td><td style={C.td}>{br?<span style={{fontSize:10,color:'#4A90D9',background:'rgba(74,144,217,0.1)',padding:'2px 8px',borderRadius:20}}>{br.name}</span>:<span style={{fontSize:10,color:'#555'}}>Not assigned</span>}</td><td style={C.td}><select className="mb-inp" style={{padding:'5px 10px',fontSize:11}} value={p.branch_id||''} onChange={e=>assignBranch(p.id,e.target.value)}><option value="">â€” select â€”</option>{branches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></td><td style={C.td}><input type="number" style={{background:'#1A1A1A',border:'1px solid #252525',color:'#F0F0F0',padding:'4px 8px',borderRadius:6,fontSize:11,width:80,fontFamily:'inherit'}} defaultValue={p.rate||150} onBlur={async e=>{ const rate=parseFloat(e.target.value)||150; await supabase.from('profiles').update({rate}).eq('id',p.id); showAlert(`âœ“ Rate updated for ${p.full_name}`,'success'); }}/></td><td style={C.td}><input type="number" min="0" style={{background:'#1A1A1A',border:'1px solid #252525',color:'#F0F0F0',padding:'4px 8px',borderRadius:6,fontSize:11,width:80,fontFamily:'inherit'}} defaultValue={p.monthly_target||0} onBlur={async e=>{ const target=parseInt(e.target.value)||0; await supabase.from('profiles').update({monthly_target:target}).eq('id',p.id); showAlert(`âœ“ Target updated for ${p.full_name}`,'success'); }} placeholder="0"/></td><td style={C.td}><button style={{...C.btn('r'),fontSize:10,padding:'3px 10px'}} onClick={()=>removeStaff(p.id,p.full_name)}>Remove</button></td></tr>); })}</tbody></table></div>
          </div>
        ...
        {showInvite&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowInvite(false)}>
          <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:32,width:'100%',maxWidth:460}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{fontSize:11,color:'#555',marginBottom:24}}>Create an account. The temporary password will be shown after â€” share it with the staff member..</div>
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              <div>
                ...
                <input className="mb-inp" type="text"
                  placeholder={
                    ['attorney','branch_manager','manager'].includes(inviteForm.role) ? 'e.g. Senior Attorney, Partner, Candidate Attorneyâ€¦' :
                    inviteForm.role==='bookkeeper' ? 'e.g. Senior Bookkeeper, Accounts Clerkâ€¦' :
                    inviteForm.role==='receptionist' ? 'e.g. Office Manager, Receptionistâ€¦' :
                    inviteForm.role==='hr' ? 'e.g. HR Manager, HR Officerâ€¦' : 'Enter titleâ€¦'
                  }
        ...
        {/* â”€â”€ MATTERS TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {tab==='matters'&&(<div style={C.main}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
            <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>All Matters</div><div style={{fontSize:11,color:'#444'}}>{matters.length} matters Â· firm-wide</div></div>
          ...
          {!matters.length?(<div style={{...C.card,textAlign:'center',padding:40,color:'#555'}}><div style={{fontSize:28,marginBottom:10}}>ðŸ“</div><div>No matters yet</div></div>):matters.filter(m=>selBranch==='all'||profiles.find(p=>p.id===m.user_id)?.branch_id===selBranch).map(m=>{
            ...
            return(<div key={m.id} style={{...C.card,border:prescWarn?'1px solid rgba(220,80,80,0.4)':m.status==='closed'?'1px solid #1A1A1A':'1px solid #1A1A1A',opacity:m.status==='closed'?0.6:1}}>
              <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}>
                <div style={{flex:1}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4,flexWrap:'wrap'}}>
                    ...
                    {prescWarn&&<span style={{fontSize:9,color:'#E05252',border:'1px solid rgba(220,80,80,0.4)',padding:'1px 8px',borderRadius:20}}>âš  Prescribes in {prescDue}d</span>}
                  ...
                  <div style={{fontSize:12,color:'#888'}}>Client: <strong style={{color:'#C0C0C0'}}>{m.client}</strong> Â· Attorney: <span style={{color:'#4A90D9'}}>{atty?.full_name||'â€”'}</span></div>
                ...
                <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'flex-start'}}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:5}}>
                    <button style={{...C.btn(),fontSize:11,padding:'4px 10px'}} onClick={()=>{const nid=mgrNotesMatter===m.id?null:m.id;setMgrNotesMatter(nid);if(nid&&!mgrNotesMap[m.id])loadMgrNotes(m.id);}}>ðŸ“ {mgrNotesMap[m.id]?.length>0?`Notes (${mgrNotesMap[m.id].length})`:'Notes'}</button>
                    <button style={{...C.btn(),fontSize:11,padding:'4px 10px'}} onClick={()=>{setOppMatter(m);setOppForm({opposing_party:m.opposing_party||'',opposing_attorney:m.opposing_attorney||'',opposing_firm:m.opposing_firm||''});setShowOppForm(true);}}>âš” Opposing</button>
                    ...
                    {m.status==='closed'&&<><button style={{...C.btn(),fontSize:11,padding:'4px 10px'}} onClick={async()=>{await updateMatter(m.id,{status:'open',closed_at:null,closed_by:null});load();}}>Reopen</button><button style={{...C.btn('p'),fontSize:11,padding:'4px 10px'}} onClick={()=>sendSatisfactionRequest(m)}>â­ Rate</button></>}
                  </div>
                </div>
              </div>
              {mgrNotesMatter===m.id&&(<div style={{borderTop:'1px solid #1A1A1A',marginTop:12,paddingTop:12}}>
                <div style={{display:'flex',gap:8,marginBottom:10}}>
                  ...
                  <button style={{...C.btn('p'),padding:'6px 12px',flexShrink:0,alignSelf:'flex-start'}} disabled={savingMgrNote||!mgrNoteText.trim()} onClick={()=>handleSaveMgrNote(m.id)}>{savingMgrNote?'â€¦':'Save'}</button>
                </div>
                ...
                {(mgrNotesMap[m.id]||[]).map(n=>(<div key={n.id} style={{display:'flex',gap:8,marginBottom:6,padding:'8px 10px',background:'#0D0D0D',borderRadius:6}}>
                  ...
                  <button style={{background:'none',border:'none',color:'#2A2A2A',cursor:'pointer',fontSize:14,flexShrink:0}} onClick={async()=>{await deleteMatterNote(n.id);loadMgrNotes(m.id);}}>âœ•</button>
                </div>))}
              </div>)}
            </div>);
          })}
        ...
        {/* â”€â”€ VAT REPORT TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {tab==='vat'&&(<div style={C.main}>
          {(()=>{
            ...
            return(<>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
                <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>VAT Report â€” VAT201 Supporting Schedule</div><div style={{fontSize:11,color:'#444'}}>Output VAT collected minus Input VAT on disbursements</div></div>
              ...
              <div style={C.card}><div style={{fontSize:12,fontWeight:600,color:'#D0D0D0',marginBottom:12}}>Output VAT â€” Invoices ({new Date(vatPeriod+'-01T12:00:00').toLocaleDateString('en-ZA',{month:'long',year:'numeric'})})</div>
        ...
        {/* â”€â”€ UNDERTAKINGS TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
              ...
              <div style={C.card}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Matter','Direction','Description','Given To','Due Date','Status','Actions'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
                {undertakings.map(u=>{
                  ...
                  return(<tr key={u.id}>
                    ...
                    <td style={{...C.td,fontFamily:'monospace',color:isOverdue?'#E05252':'#888',fontWeight:isOverdue?700:400}}>{u.due_date?fmtDate(u.due_date):'â€”'}{isOverdue&&' âš '}</td>
        ...
        {/* â”€â”€ COMMUNICATIONS TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
          ...
          <div style={C.card}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Date','Type','Direction','Client','Matter','Subject','Logged By','Actions'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
            {communications.map(c=>{
              ...
              return(<tr key={c.id}>
                ...
                <td style={{...C.td,color:'#C8C8C8'}}>{cl?.full_name||'â€”'}</td>
                ...
                <td style={{...C.td,fontSize:10,color:'#555'}}>{c.profiles?.full_name||'â€”'}</td>
        ...
        {/* â”€â”€ INTEREST TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {tab==='interest'&&(<div style={C.main}>
          {(()=>{
            ...
            return(<>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
                <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Interest on Overdue Accounts</div><div style={{fontSize:11,color:'#444'}}>Legal rate: {RATE}% per annum Â· Invoices overdue &gt;30 days</div></div>
              ...
              {!overdueInvs.length?(<div style={{...C.card,textAlign:'center',padding:40,color:'#555'}}><div style={{fontSize:28,marginBottom:10}}>âœ…</div><div>No overdue invoices</div></div>):
        ...
        {/* â”€â”€ AUDIT LOG TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {tab==='audit'&&(<div style={C.main}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
            ...
            <button style={C.btn()} onClick={()=>{ setAuditLoading(true); fetchAuditLog({}).then(r=>{setAuditLogs(r.logs||[]);setAuditLoading(false);}); }}>â†» Refresh</button>
          </div>
          {auditLoading?(<div style={{...C.card,textAlign:'center',padding:40,color:'#555'}}>Loadingâ€¦</div>):
          !auditLogs.length?(<div style={{...C.card,textAlign:'center',padding:40,color:'#555'}}><div style={{fontSize:28,marginBottom:10}}>ðŸ“‹</div><div>No audit entries yet</div><div style={{fontSize:11,color:'#333',marginTop:6}}>Actions like invoicing, trust transactions and credit notes will appear here</div></div>):
          <div style={C.card}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Date & Time','User','Action','Entity','Details'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
            {auditLogs.map(l=>(<tr key={l.id}>
              ...
              <td style={{...C.td,fontSize:10,color:'#A78BFA',fontFamily:'monospace'}}>{l.entity_type} {l.entity_id?`Â· ${l.entity_id.substring(0,12)}â€¦`:''}</td>
              <td style={{...C.td,fontSize:10,color:'#555'}}>{l.details?JSON.stringify(l.details).substring(0,60):'â€”'}</td>
            </tr>))}
          </tbody></table></div>}
        </div>)}

        {/* â”€â”€ CAMPAIGNS TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {tab==='campaigns'&&(<div style={C.main}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
              <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Client Campaigns</div><div style={{fontSize:11,color:'#444'}}>Send bulk emails to clients â€” overdue notices, newsletters, announcements</div></div>
            </div>
        ...
        {/* â”€â”€ COURT ROLL TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {tab==='courtroll'&&(<div style={C.main}>
          ...
          {(()=>{
            ...
            return(<>
              {!sorted.length?(<div style={{...C.card,textAlign:'center',padding:40,color:'#555'}}><div style={{fontSize:28,marginBottom:10}}>âš–ï¸</div><div style={{fontSize:14}}>No court dates or action dates set</div><div style={{fontSize:11,color:'#333',marginTop:6}}>Set prescription dates and next action dates on matters to see them here</div></div>):
                ...
                {sorted.map(m=>{
                  ...
                  return dates.map((d,i)=>{
                    ...
                    return(<tr key={m.id+i} style={{background:d.urgent?'rgba(220,80,80,0.04)':''}}>
                      ...
                      <td style={{...C.td,color:'#4A90D9',fontSize:11}}>{atty?.full_name||'â€”'}</td>
        ...
        {/* â”€â”€ DOCUMENT TEMPLATES TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {tab==='templates'&&(<div style={C.main}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
            <div><div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em'}}>Document Templates</div><div style={{fontSize:11,color:'#444'}}>Generate standard firm documents â€” select matter to pre-fill details</div></div>
          </div>
          <div style={{...C.card,marginBottom:14}}>
            <div style={{fontSize:12,fontWeight:600,color:'#D0D0D0',marginBottom:10}}>Select Matter (optional â€” pre-fills client and attorney details)</div>
            <select style={{...C.sel,maxWidth:500}} value={selTemplate} onChange={e=>setSelTemplate(e.target.value)}>
              <option value="">â€” No matter selected (blank template) â€”</option>
              {matters.map(m=><option key={m.id} value={m.id}>{m.id} Â· {m.name} â€” {m.client}</option>)}
            </select>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:12}}>
            {[
              {id:'engagement',label:'Letter of Engagement',icon:'ðŸ“‹',desc:"Formal letter setting out the firm's mandate and fee arrangement"},
              {id:'demand',label:'Letter of Demand',icon:'âš ï¸',desc:'Formal demand for payment or performance within 7 days'},
              {id:'trust_receipt',label:'Trust Receipt',icon:'ðŸ¦',desc:'Acknowledgement of funds received into the trust account'},
              {id:'withdrawal',label:'Notice of Withdrawal',icon:'ðŸ“„',desc:'Notice of withdrawal as attorney of record'},
            ].map(t=>{
              ...
              const hdr=`<div style="text-align:center;border-bottom:2px solid #000;padding-bottom:12px;margin-bottom:20px"><strong style="font-size:18px">Motsoeneng Bill</strong><br/><small>Attorneys Â· Notaries Â· Conveyancers</small></div>`;
        ...
        {/* â”€â”€ CLIENT REQUESTS TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {tab==='requests'&&(<div style={C.main}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
            ...
            <button style={C.btn()} onClick={()=>supabase.from('client_requests').select('*').order('created_at',{ascending:false}).then(({data})=>setClientRequests(data||[]))}>â†» Refresh</button>
          </div>
          {(()=>{
            ...
            return(<>
              ...
              {!clientRequests.length?(<div style={{...C.card,textAlign:'center',padding:40,color:'#555'}}><div style={{fontSize:28,marginBottom:10}}>ðŸ“¬</div><div style={{fontSize:14}}>No requests yet</div><div style={{fontSize:11,color:'#333',marginTop:6}}>Client requests submitted via the portal or request form will appear here</div></div>):
              <div style={C.card}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>{['Date','Name','Email','Phone','Service','Urgency','Status','Actions'].map(h=><th key={h} style={C.th}>{h}</th>)}</tr></thead><tbody>
                {clientRequests.map(r=>(<tr key={r.id}>
                  ...
                  <td style={{...C.td,fontSize:11,color:'#555'}}>{r.phone||'â€”'}</td>
                  ...
                  <td style={C.td}><div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
                    {r.status==='pending'&&(<>
                      <button style={{...C.btn('p'),fontSize:10,padding:'3px 8px'}} onClick={async()=>{
                        ...
                        showAlert(`âœ“ Request from ${r.full_name} marked as reviewed.`);
                      }}>Review</button>
                      <button style={{...C.btn(),fontSize:10,padding:'3px 8px'}} onClick={async()=>{
                        // Convert to client
                        router.push('/clients');
                        showAlert(`Opening clients â€” add ${r.full_name} (${r.email}) as a new client.`,'success');
                      }}>â†’ Client</button>
                    </>)}
                    {r.status==='reviewed'&&(
                      <button style={{...C.btn('p'),fontSize:10,padding:'3px 8px'}} onClick={async()=>{
                        ...
                        showAlert(`âœ“ Marked as converted.`);
                      }}>âœ“ Converted</button>
                    )}
        ...
        {/* â”€â”€ MODALS: Undertaking form â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {showUTForm&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowUTForm(false)}>
          <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:28,width:'100%',maxWidth:500}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                <div><label style={lbl}>Matter *</label><select className="mb-inp" value={utForm.matter_id} onChange={e=>setUtForm(f=>({...f,matter_id:e.target.value}))}><option value="">â€” Select â€”</option>{matters.map(m=><option key={m.id} value={m.id}>{m.id} Â· {m.client}</option>)}</select></div>
        ...
        {/* â”€â”€ MODAL: Communication form â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {showCommForm&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowCommForm(false)}>
          <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:28,width:'100%',maxWidth:520}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                <div><label style={lbl}>Client</label><select className="mb-inp" value={commForm.client_id} onChange={e=>setCommForm(f=>({...f,client_id:e.target.value}))}><option value="">â€” Select client â€”</option>{clients.map(c=><option key={c.id} value={c.id}>{c.full_name}</option>)}</select></div>
                <div><label style={lbl}>Matter</label><select className="mb-inp" value={commForm.matter_id} onChange={e=>setCommForm(f=>({...f,matter_id:e.target.value}))}><option value="">â€” Select matter â€”</option>{matters.filter(m=>!commForm.client_id||clients.find(c=>c.id===commForm.client_id)?.full_name===m.client).map(m=><option key={m.id} value={m.id}>{m.id} Â· {m.name}</option>)}</select></div>
              </div>
        ...
        {/* â”€â”€ MODAL: Matter closure â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {closingMatter&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setClosingMatter(null)}>
          <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:28,width:'100%',maxWidth:440}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:15,fontWeight:700,marginBottom:4}}>Close Matter â€” {closingMatter.id}</div>
            <div style={{fontSize:11,color:'#555',marginBottom:16}}>{closingMatter.name} Â· {closingMatter.client}</div>
        ...
        {/* â”€â”€ OPPOSING PARTY MODAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {showOppForm&&oppMatter&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowOppForm(false)}>
          <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:28,width:'100%',maxWidth:460}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:15,fontWeight:700,marginBottom:4}}>Opposing Party â€” {oppMatter.id}</div>
            <div style={{fontSize:11,color:'#555',marginBottom:16}}>{oppMatter.name} Â· {oppMatter.client}</div>
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
        ...
        {/* â”€â”€ PAYMENT PLAN MODAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        {showPayPlan&&payPlanInv&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowPayPlan(false)}>
          <div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:28,width:'100%',maxWidth:440}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:15,fontWeight:700,marginBottom:4}}>Payment Plan â€” {payPlanInv.id}</div>
            <div style={{fontSize:11,color:'#555',marginBottom:4}}>{payPlanInv.client} Â· Total: {fmtR((payPlanInv.total_units||0)*(payPlanInv.rate||150)*1.15)} incl. VAT</div>
            {payPlanForm.instalment&&(<div style={{fontSize:11,color:'#8DC63F',marginBottom:12}}>â‰ˆ {Math.ceil((payPlanInv.total_units||0)*(payPlanInv.rate||150)*1.15/parseFloat(payPlanForm.instalment))} instalments of R{payPlanForm.instalment}</div>)}
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            ...
            <div style={{display:'flex',gap:8,marginTop:16,justifyContent:'flex-end'}}>
              ...
              <button style={C.btn('p')} disabled={savingPlan||!payPlanForm.instalment} onClick={handleSavePayPlan}>{savingPlan?'Savingâ€¦':'Create Plan'}</button>
            </div>
          </div>
        </div>)}

        {showCNForm&&cnInvoice&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowCNForm(false)}><div style={{background:'#111',border:'1px solid #2A2A2A',borderRadius:12,padding:28,width:'100%',maxWidth:420}} onClick={e=>e.stopPropagation()}><div style={{fontSize:15,fontWeight:700,marginBottom:4}}>Issue Credit Note</div><div style={{fontSize:11,color:'#555',marginBottom:16}}>Against invoice <strong style={{color:'#F0F0F0'}}>{cnInvoice.id}</strong> Â· {cnInvoice.client} Â· {fmtR((cnInvoice.total_units||0)*(cnInvoice.rate||150)*1.15)} incl. VAT</div><div style={{display:'flex',flexDirection:'column',gap:12}}><div><label style={lbl}>Credit amount (R) *</label><input className="mb-inp" type="number" placeholder="0.00" value={cnForm.amount} onChange={e=>setCnForm(f=>({...f,amount:e.target.value}))}/></div><div><label style={lbl}>Reason *</label><textarea className="mb-inp" style={{minHeight:70,resize:'vertical'}} placeholder="Reason for credit note..." value={cnForm.reason} onChange={e=>setCnForm(f=>({...f,reason:e.target.value}))}/></div></div><div style={{display:'flex',gap:8,marginTop:16,justifyContent:'flex-end'}}><button style={C.btn()} onClick={()=>setShowCNForm(false)}>Cancel</button><button style={C.btn('p')} disabled={savingCN||!cnForm.amount||!cnForm.reason.trim()} onClick={handleSaveCreditNote}>{savingCN?'Savingâ€¦':'Issue Credit Note'}</button></div></div></div>)}
