...
export default function Manager() {
  ...
  async function removeStaff(userId, name) { if (!confirm(`Remove ${name} from the system? This cannot be undone.`)) return; const res = await fetch('/api/remove-staff', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId }) }); const data = await res.json(); if (!res.ok) { showAlert('Error: ' + data.error, 'error'); return; } showAlert(`✓ ${name} removed.`); load(); }
