...
export default function Bookkeeper() {
  ...
  async function postReceipt(){
    ...
    showTrustAlert(`âœ“ Receipt ${receiptNo} posted â€” ${fmtR(amount)}`,'success');
  ...
  async function postPayment(){
    ...
    if(amount>bal){ showTrustAlert(`âœ— Insufficient balance. Available: ${fmtR(bal)}.`,'error'); return; }
    setTrustSaving(true);
    ...
    if(error){ showTrustAlert('Error: '+error.message,'error'); setTrustSaving(false); return; }
    showTrustAlert(needsApproval?`â³ Payment of ${fmtR(amount)} submitted for approval.`:`âœ“ Payment of ${fmtR(amount)} to ${pForm.payee} posted.`,'success');
  ...
  async function postTransfer(){
    ...
    setTrustSaving(true);
    const {error}=await supabase.from('trust_transactions').insert([{type:'transfer',matter_id:tForm.matterId,user_id:profile?.id,date:tForm.date,amount:totalAmount,amount_excl_vat:amountExclVAT,vat_amount:vatAmount,trust_account_id:tForm.fromAccountId||null,to_account:tForm.toAccount,invoice_id:tForm.invoiceId,narration:tForm.narration||`Transfer of fees â€” ${tForm.matterId}`,captured_by:profile?.id,branch_id:tForm.branchId||null,status:'posted'}]);
    if(error){ showTrustAlert('Error: '+error.message,'error'); setTrustSaving(false); return; }
    showTrustAlert(`âœ“ Transfer posted â€” Fees: ${fmtR(amountExclVAT)} + VAT: ${fmtR(vatAmount)} = Total: ${fmtR(totalAmount)}`,'success');
    setTForm(f=>({...f,amount:'',matterId:'',invoiceId:'',narration:''}));
    setTBalanceCheck(null); setTrustSaving(false); loadTrust();
  }

  async function lockPeriod(period){ if(!confirm(`Lock period ${period}?`)) return; const {error}=await supabase.from('trust_period_locks').insert([{period,locked_by:profile?.id}]); if(error){ showTrustAlert('Error: '+error.message,'error'); return; } showTrustAlert(`âœ“ Period ${period} locked.`,'success'); loadTrust(); }
  ...
  function printTrustStatement(matter, transactions) {
    let running=0;
    const rows=transactions.map(t=>{ const isR=t.type==='receipt'; if(isR) running+=Number(t.amount); else running-=Number(t.amount); return `<tr><td>${fmtDate(t.date)}</td><td style="text-transform:capitalize">${t.type}</td><td>${t.receipt_no||t.reference||'â€”'}</td><td>${t.narration||''}</td><td align="right" style="color:#dc2626">${!isR?fmtR(t.amount):''}</td><td align="right" style="color:#16a34a">${isR?fmtR(t.amount):''}</td><td align="right" style="font-weight:700;color:${running>=0?'#16a34a':'#dc2626'}">${fmtR(running)}</td></tr>`; }).join('');
    const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Trust Statement</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;color:#111;padding:40px;max-width:820px;margin:auto}table{width:100%;border-collapse:collapse;margin:16px 0}th{background:#f8f8f8;padding:8px;font-size:9px;text-transform:uppercase;color:#aaa;border-bottom:2px solid #eee;text-align:left}td{padding:7px 8px;font-size:11px;border-bottom:1px solid #f3f3f3}.foot{margin-top:20px;padding-top:12px;border-top:1px solid #eee;font-size:10px;color:#ccc;text-align:center}</style></head><body><h2 style="color:#8DC63F;margin-bottom:4px">Motsoeneng Bill</h2><h3>Trust Account Statement â€” ${matter?.id}</h3><p style="color:#888;font-size:12px;margin-bottom:16px">Client: ${matter?.client} Â· Balance: ${fmtR(running)}</p><table><thead><tr><th>Date</th><th>Type</th><th>Reference</th><th>Description</th><th align="right">Debit</th><th align="right">Credit</th><th align="right">Balance</th></tr></thead><tbody>${rows}</tbody></table><div class="foot">Motsoeneng Bill Â· VAT: 4100000000 Â· accounts@mb.co.za</div><script>window.onload=function(){window.print();}<\/script></body></html>`;
  ...
  return(
    <>
      <Head><title>MB SmartTrack â€” Bookkeeper</title></Head>
      ...
      <div style={C.page}>
        ...
        {pendingPayments.length>0&&tab!=='trust'&&(<div style={{background:'rgba(234,179,8,0.1)',border:'1px solid rgba(234,179,8,0.3)',padding:'10px 24px',fontSize:12,color:'#EAB308',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span>â³ {pendingPayments.length} payment{pendingPayments.length>1?'s':''} pending approval â€” {fmtR(pendingPayments.reduce((s,t)=>s+Number(t.amount),0))}</span><button style={C.btn('warn')} onClick={()=>setTab('trust')}>Review â†’</button></div>)}

        {tab==='trust'&&(<div style={C.main}>
          <div style={{fontSize:16,fontWeight:700,letterSpacing:'-0.03em',marginBottom:4}}>ðŸ¦ Trust Accounting</div>
          ...
          <div style={{display:'flex',gap:6,marginBottom:20,flexWrap:'wrap'}}>
            {[['ledger','ðŸ“Š Ledger'],['receipt','â¬‡ Receipt'],['payment','â¬† Payment'],['transfer','â†” Transfer'],['recon','ðŸ” Reconciliation'],['reports','ðŸ“‹ Reports']].map(([v,l])=>(
          ...
          {trustTab==='receipt'&&(<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
            <div style={C.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0'}}>New trust receipt</div><span style={{fontSize:10,color:'#4A90D9',border:'1px solid rgba(74,144,217,0.3)',padding:'2px 10px',borderRadius:20}}>Next: {nextReceiptNo(trustTransactions)}</span></div>
              <div style={{display:'flex',flexDirection:'column',gap:12}}>
                ...
                <div><label style={C.lbl}>Matter *</label><select style={C.tinp} value={rForm.matterId} onChange={e=>setRForm(f=>({...f,matterId:e.target.value}))}><option value="">Select matter...</option>{allMatters.map(m=><option key={m.id} value={m.id}>{m.id} â€” {m.client}</option>)}</select></div>
          ...
          {trustTab==='transfer'&&(<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
            <div style={C.card}><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0',marginBottom:16}}>Trust to business transfer</div>
              <div style={{display:'flex',flexDirection:'column',gap:12}}>
                ...
                <div><label style={C.lbl}>Matter *</label><select style={C.tinp} value={tForm.matterId} onChange={e=>{ setTForm(f=>({...f,matterId:e.target.value,invoiceId:'',amount:''})); checkTransferBalance(e.target.value,tForm.amount); }}><option value="">Select matter...</option>{allMatters.map(m=><option key={m.id} value={m.id}>{m.id} â€” {m.client} (bal: {fmtR(getMatterBalance(m.id))})</option>)}</select></div>
                {tBalanceCheck&&(<div style={{background:tBalanceCheck.ok?'rgba(141,198,63,0.08)':'rgba(220,80,80,0.08)',border:`1px solid ${tBalanceCheck.ok?'rgba(141,198,63,0.3)':'rgba(220,80,80,0.3)'}`,borderRadius:6,padding:'8px 12px',fontSize:12,color:tBalanceCheck.ok?'#8DC63F':'#E05252'}}>{tBalanceCheck.ok?<div><div>âœ“ Available: {fmtR(tBalanceCheck.bal)}</div><div style={{fontSize:11,color:'#888',marginTop:4}}>Excl. VAT: {fmtR(tBalanceCheck.amt)} + VAT 15%: {fmtR(tBalanceCheck.amt*0.15)} = <strong style={{color:'#EAB308'}}>Total: {fmtR(tBalanceCheck.amt*1.15)}</strong></div><div style={{fontSize:11,color:'#555',marginTop:2}}>After: {fmtR(tBalanceCheck.bal-tBalanceCheck.amt*1.15)}</div></div>:`âœ— Insufficient â€” available: ${fmtR(tBalanceCheck.bal)}`}</div>)}
                {tForm.matterId&&getMatterInvoices(tForm.matterId).length>0&&(<div><label style={C.lbl}>Link to invoice</label><select style={C.tinp} value={tForm.invoiceId} onChange={e=>{ const inv=invoices.find(i=>i.id===e.target.value); setTForm(f=>({...f,invoiceId:e.target.value,narration:inv?`Transfer of fees â€” ${inv.matter_name} â€” ${inv.id}`:''})); }}><option value="">Select invoice (optional)...</option>{getMatterInvoices(tForm.matterId).map(i=><option key={i.id} value={i.id}>{i.id} â€” R{((i.total_units||0)*(i.rate||150)*1.15).toFixed(2)} incl. VAT</option>)}</select></div>)}
        ...
        {tab==='wip'&&(<div style={C.main}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
            ...
            <button style={C.btn()} onClick={loadData}>â†» Refresh</button>
          </div>
          ...
          {!wipData.length?(<div style={{...C.card,textAlign:'center',padding:'40px',color:'#555'}}><div style={{fontSize:28,marginBottom:10}}>âœ…</div><div style={{fontSize:14}}>All billable work has been invoiced</div></div>):wipData.map(p=>(<div key={p.id} style={C.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12,flexWrap:'wrap',gap:8}}><div><div style={{fontSize:13,fontWeight:600,color:'#D0D0D0'}}>{p.full_name}</div><div style={{fontSize:10,color:'#555'}}>{branches.find(b=>b.id===p.branch_id)?.name||'â€”'} Â· R{p.attyRate}/unit</div></div><div style={{display:'flex',gap:16}}><div style={{textAlign:'center'}}><div style={{fontSize:9,color:'#555',textTransform:'uppercase',marginBottom:2}}>Earned</div><div style={{fontSize:16,fontWeight:700,color:'#888'}}>{p.earnedUnits}</div></div><div style={{textAlign:'center'}}><div style={{fontSize:9,color:'#555',textTransform:'uppercase',marginBottom:2}}>Billed</div><div style={{fontSize:16,fontWeight:700,color:'#8DC63F'}}>{p.billedUnits}</div></div><div style={{textAlign:'center'}}><div style={{fontSize:9,color:'#555',textTransform:'uppercase',marginBottom:2}}>Unbilled</div><div style={{fontSize:16,fontWeight:700,color:'#EAB308'}}>{p.unbilledUnits}</div></div><div style={{textAlign:'center'}}><div style={{fontSize:9,color:'#555',textTransform:'uppercase',marginBottom:2}}>Est. Value</div><div style={{fontSize:16,fontWeight:700,color:'#EAB308'}}>R{p.estValue.toLocaleString()}</div></div></div></div>
