...
export default function NavBar({ role, tab, setTab, onSignOut, profile, clock, pendingCount=0, ficaCount=0, rightSlot }) {
  ...
  const S = {
    ...
    nav: {
      display:'flex', alignItems:'center', gap:2, flex:1, flexWrap:'nowrap', overflowX:'auto',
    },
    item: (active) => ({
  ...
  return (
    <div style={S.hdr} ref={ref}>
      ...
      <div style={S.logo}>
        {firm.logo_url
          ? <img src={firm.logo_url} alt="" style={{width:30,height:30,objectFit:'contain',borderRadius:5}}/>
          : <div style={{width:30,height:30,background:'#8DC63F',borderRadius:5,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:900,fontSize:11,color:'#0A0A0A'}}>MB</div>
        }
